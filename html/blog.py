...
cgitb.enable(display=0, logdir="/var/www/html/temp")
import smtplib
...
def generateDate(fileName):
    ...
    date = "%s %s:%s:%s" % (date, filedate.hour, filedate.minute, filedate.second)
    try:
        date = datetime(*strptime(date, "%Y-%m-%d %H:%M:%S")[0:6])
    except:
        date = filedate
...
def search(pattern, lines):
    ...
    for line in lines:
        ...
        if m:
            ...
            matchlines.append(
                '%04d: %s<div id="hit">%s</div>%s\n'
                % (linenumber, startline, middleline, endline)
            )
        linenumber += 1
    return matchlines
...
class Comment:
    ...
    def getAuthorLink(self):
        ...
        if url is None:
            ...
            return '<a href="%s"  rel="external nofollow">%s</a>' % (url, self.author)

    def getText(self):
        comment = str(self.comment)
        comment = self.comment.replace("\r\n", "<br />")
        comment = self.urlre.sub(r'<a href="\1://\2">\1://\2</a>', comment)
        return comment
...
def handleIncomingComment(fs):
    ...
    if subscribe and email in getSubscribedEmails(comments_for_entry).keys():
        subscribe = None

    new_comment = pickleComment(author, email, url, comment, name, indexdir, subscribe)
    ...
    try:
        ...
        if subscribe:
            email_body_admin += (
                "\n\nNote: commenter subscribed (%s) to follow-up comments" % email
            )
...
class Entry:
    ...
    def __init__(self, fileName, datadir):
        ...
        self.url = "%s/%s" % (baseurl, quote_plus(self.fileName[:-4]))
    ...
    def getFirstParagraph(self):
        ...
        if m is not None:
            there_is_more = False
            there_is_more_str = '<a href="%s">%s</a>' % (self.url, l_read_more)
...
class Entries:
    ...
    def getMany(self, pagenumber=0, cat=None, numberofentriesperpage=10):
        ...
        if pagenumber == -1:  # no limit
            ...
            swd = list(swd)[sindex:eindex]
        else:
            swd = list(swd)[:numberofentriesperpage]
        for key in swd:
...
def renderHtmlFooter():
    print(
        '<div id="footer">Powered by <a href="http://23.fi/kukkaisvoima">Kukkaisvoima</a> version %s</div>'
        % version
    )
    print("</div>")  # content1
    ################################
...
def renderHtmlHeader(title=None, links=[]):
    ...
    if sess != "":
        ...
        cursor = conn.cursor()
        cursor.execute(
            'SELECT userID, emailAddress FROM tUsers WHERE userID="' + currentUser + '"'
        )
        row = cursor.fetchone()
        if row != None:
            emailAddr = row[1]
    ...
    print(
        '<link rel="alternate" type="application/rss+xml" title="%s RSS Feed" href="%s/feed/" />'
        % (blogname, baseurl)
    )
...
def renderComment(entry, comment, numofcomment, admin=False, pretext=False):
    print("<li>")
    if gravatarsupport:
        print('<img style="padding-right:5px;"')
        print(
            'src="http://gravatar.com/avatar/%s?s=40&d=identicon" align="left"/>'
    ...
    print(
        '<small><a name ="comment-%s" href="%s#comment-%s">%s</a> %s </small>'
        % (numofcomment, entry.url, numofcomment, dateToString(comment.date), delcom)
    )
    if pretext:
        print(pretext)
    else:
...
def renderEntryLinks(entries, text=None, comment_tuple_list=None):
    ...
    for entry in entries:
        link = '<li><a href="%s">%s</a>' % (entry.url, entry.headline)
        index = 1
        for cat in entry.cat:
            link += "%s" % cat
...
def renderCategories(catelist, ent, path):
    ...
    if len(path) == 1 and path[0] == "categories":
        ...
        sortedcat = sorted(sortedcat, key=locale.strxfrm)
...
def renderSearch(entries, searchstring):
    ...
    for entry in entries:
        ...
        if (
            len(matchedfiles[entry]["lines"]) == 0
            and len(matchedfiles[entry]["headline"]) == 0
            and comments_matches == False
        ):
    ...
    for entry in matchedfiles.keys():
        ...
        for comment in matchedfiles[entry]["comments"].keys():
            ...
            for line in matchedfiles[entry]["comments"][comment]["lines"]:
                pline += line
            com_list.append(
                (comment, pline, matchedfiles[entry]["comments"][comment]["author"])
            )
        pline = ""
...
def renderDeleteComments(entry, commentnum):
    ...
    print(
        '<form action="%s/%s/?deletecomment" method="post" id="deleteform">'
        % (baseurl, quote_plus(entry.fileName[:-4]))
    )
    print(
        '<input type="hidden" name="commentnum" id="commentnum" value="%s"/>'
        % (commentnum)
    )
    print(
        '<input type="hidden" name="name" id="name" value="%s"/>' % entry.fileName[:-4]
    )
    print(
        '<p><input type="password" name="password" id="password" size="22" tabindex="1" />'
    )
...
def renderSidebarCategories(catelist, rss_categories):
    ...
    categories = sorted(categories, key=locale.strxfrm)
    ...
    for cat in categories:
        ...
        print(
            '<li%s><a href="%s/%s">%s</a> (%s)'
            % (add_str, baseurl, quote_plus(cat), cat, len(catelist[cat]))
        )
        if cat in rss_categories:
            print(
                '<a href="%s/%s/feed"><img alt="RSS Feed Icon" src="%s" style="vertical-align:top; border:none;"/></a>'
...
def renderSidebarCommments():
    if sidebarcomments:
        ...
        if len(comlist) == 0:
            ...
            for com in comlist:
                print(
                    '<li>%s on <a href="%s/%s#comment-%d">%s</a>'
                    % (
                        com["author"],
                        baseurl,
                        quote_plus(com["file"][:-4]),
                        com["num"],
                        com["subject"],
                    )
                )
...
def renderSidebarArchive(arclist):
    print(
        '<h2><a href="%s/archive">%s</a> (%d)</h2>'
        % (
            baseurl,
            l_archives,
            # total number of entries
            sum([len(l) for l in [i for i in arclist]]),
        )
    )
    print(l_toggle)
    print("<ul>")
    sortedarc = arclist.keys()
    sortedarc = sorted(sortedarc, reverse=True)
    ...
    years_keys = sorted(years_keys, reverse=True)

    # display each year at top lovel and if visiability is toggled
    # then show months
    for year in years_keys:
        print(
            '<li><a href="#" onclick="toggle_years(\'con-year-%s\'); return false;">%s</a> (%d)'
            % (
                year,
                year,
...
def renderSidebarAdmin(entries):
    if len(entries) == 1:
        ...
        print("<ul>")
        print(
            '<li><a href="%s/%s/?admin" rel="nofollow">%s</a>'
            % (baseurl, quote_plus(entries[0].fileName[:-4]), l_admin_comments)
        )
        print("</ul>")
...
def renderHtml(entries, path, catelist, arclist, admin, page, mode):
    ...
    for cat in categories:
        rss.append(
            '<link rel="alternate" type="application/rss+xml" title="%s: %s RSS Feed" href="%s/%s/feed/" />'
    ...
    for entry in entries:
        print('<h2><a href="%s">%s</a></h2>' % (entry.url, entry.headline))
        ...
        if len(entries) > 1 and maxcomments > -1:
            ...
            if nc > 0:
                print(
                    '<div class="comlink">%s <a href="%s#comments">%s</a></div>'
                    % (nc, entry.url, l_comments2)
                )
            else:
                print(
                    '<div class="comlink"><a href="%s#leave_acomment">%s</a></div>'
                    % (entry.url, l_no_comments)
                )
        ...
        print("</div>")
        print('<div class="date">%s: %s</div>' % (l_date, dateToString(entry.date)))
        ...
        if len(entries) == 1:
            ...
            if maxcomments == -1 or len(entry.comments) >= maxcomments:
                ...
                print(
                    '<form action="%s/%s/?postcomment" method="post"'
                    % (baseurl, entry.fileName[:-4])
                )
                print('id="commentform" onsubmit="return validate_form(this)">')  # form
                print(
                    '<input type="hidden" name="name" id="name" value="%s"/>'
                    % entry.fileName[:-4]
                )
                print(
                    '<input type="hidden" name="headline" id="headline" value="%s"/>'
                    % entry.headline
                )
                print(
                    '<input type="hidden" name="commentnum" id="commentnum" value="%s"/>'
                ...
                print(
                    '<label for="author"><small>%s</small></label></p>' % l_name_needed
                )
                print(
                    '<p><input type="text" name="email" id="email" size="22" tabindex="2" />'
                )
                print(
                    '<label for="email"><small>%s</small></label></p>' % l_email_needed
                )
                print(
                ...
                print(
                    '<label for="nospam"><small>%s</small></label></p>'
                    % l_nospam_question
                )
                ...
                print("</p>")
                print(
                    '<p><input type="checkbox" name="subscribe" id="subscribe" tabindex="6" value="subscribe">%s</label></p>'
                    % l_notify_comments
                )
                print("</form>")
    ...
    if len(entries) > 1:
        ...
        if page > 0:
            print(
                '<a href="%s/%s?page=%s">%s</a>'
                % (baseurl, "/".join(path), page - 1, l_previouspage)
            )
        if len(entries) == numberofentriesperpage:
            print(
                '<a href="%s/%s?page=%s">%s</a>'
                % (baseurl, "/".join(path), page + 1, l_nextpage)
            )
        print("</div>")
    print("</div>")  # content2
...
def renderFeed(entries, path, categorieslist, totalCount):
    ...
    print(
        ...
        % strftime(rfc822time, entries[0].date.timetuple())
    )
    print(
    ...
    if len(newarticles) > 0:
        ...
        for cat in categorieslist.keys():
            ...
            if not oldcategorieslist or (
                oldcategorieslist
                and len(
                    set(oldcategorieslist.values()) ^ set(categorieslist[cat].values())
                )
                > 0
            ):
        ...
        for arc in archivelist.keys():
            ...
            if not oldarchivelist or (
                oldarchivelist
                and len(set(oldarchivelist.values()) ^ set(archivelist[arc].values()))
                > 0
            ):
