...
result = ""
currentSpawnsSQL = (
    "SELECT Count(spawnID) FROM tResources WHERE galaxy="
    + galaxy
    + " AND unavailable IS NULL;"
)
totalSpawnsSQL = (
    "SELECT totalSpawns, galaxyName FROM tGalaxy WHERE galaxyID=" + galaxy + ";"
)
currentWaypointsSQL = (
    "SELECT Count(*) FROM tWaypoint INNER JOIN tResources ON tWaypoint.spawnID = tResources.spawnID WHERE galaxy="
    + galaxy
    + " AND tWaypoint.unavailable IS NULL AND tResources.unavailable IS NULL;"
)
