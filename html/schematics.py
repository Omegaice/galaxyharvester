...
def getComponentLink(cn, objectPath, ingredientType):
    ...
    compCursor.execute(
        'SELECT schematicID, schematicName, complexity, xpAmount, (SELECT imageName FROM tSchematicImages tsi WHERE tsi.schematicID=tSchematic.schematicID AND tsi.imageType=1) AS schemImage FROM tSchematic WHERE objectPath="'
        + objectPath
        + '" OR objectGroup="'
        + objectPath
        + '";'
...
def getProfession(conn, skillGroup):
    ...
    profCursor.execute(
        "SELECT profID FROM tSkillGroup WHERE skillGroup=%s;", [skillGroup]
    )
                ...
                if row != None:
                    ...
                    while ingRow != None:
                        ...
                        if ingRow[1] == 0:
                            ...
                            # component
                            results = getComponentLink(
                                conn, tmpObject, ingRow[1]
                            ).split("|")
                            tmpLink = results[1]
                            tmpImage = results[0]
                            tmpName = results[2]
                    ...
                    if logged_state > 0:
                        ...
                        if favRow != None:
                            favHTML = (
                                '  <div class="inlineBlock" style="width:3%;float:left;"><a alt="Favorite" title="Favorite" style="cursor: pointer;" onclick="toggleFavorite(this, 4, \''
                                + schematicID
