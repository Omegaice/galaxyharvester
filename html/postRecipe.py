...
def addRecipe(conn, schematicID, recipeName, user, galaxy):
    ...
    tempSQL = (
        "SELECT schematicName FROM tSchematic WHERE schematicID='" + schematicID + "';"
    )
    try:
    ...
    if returnStr.find("Error:") == -1:
        tempSQL = (
            "INSERT INTO tRecipe (recipeName, userID, schematicID, galaxy) VALUES ('"
            + recipeName
            + "','"
            + user
            + "','"
...
def updateIngredients(conn, recipeID, ingredients, schematicID, user):
    ...
    for ingredient in ingredients:
        if ingredient != "" and ingredient != "clear":
            ...
            if (
                len(ingredientVals) == 2
                and ingredientVals[0] != ""
                and ingredientVals[1] != ""
            ):
                ingredientNames += "'" + ingredientVals[0] + "',"
                ingredientQuality = None
                # See if ingredient slot already exists
                tempSQL = (
                    "SELECT ingredientResource FROM tRecipeIngredients WHERE recipeID="
                    + str(recipeID)
                    + " AND ingredientName='"
                    + ingredientVals[0]
                    + "';"
                ...
                if row != None:
                    ...
                    if row[0] != ingredientVals[1]:
                        ingredientQuality = recipe.calcIngredientQuality(
                            conn, ingredientVals[1], schematicID, user
                        )
                        tempSQL = (
                            "UPDATE tRecipeIngredients SET ingredientResource="
                            + ingredientVals[1]
                            + ", ingredientQuality="
                            + n2n(ingredientQuality)
                            + " WHERE recipeID="
                            + str(recipeID)
                            + " AND ingredientName='"
                            + ingredientVals[0]
                    ...
                    tempSQL = (
                        "INSERT INTO tRecipeIngredients (recipeID, ingredientName, ingredientResource, ingredientQuality) VALUES ("
                        + str(recipeID)
                        + ",'"
                        + ingredientVals[0]
                        + "',"
                        + str(ingredientVals[1])
                        + ","
                        + n2n(ingredientQuality)
                        + ");"
                    )
    ...
    ingredientNames += "''"
    tempSQL = (
        "DELETE FROM tRecipeIngredients WHERE recipeID="
        + str(recipeID)
        + " AND ingredientName NOT IN ("
        + ingredientNames
        + ");"
    )
    cursor.execute(tempSQL)
...
def addIngredient(conn, recipeID, spawnID, schematicID, user):
    ...
    ingSql = (
        "SELECT si.ingredientName, ingredientObject, ingredientResource FROM tSchematicIngredients si LEFT JOIN (SELECT ingredientName, ingredientResource FROM tRecipeIngredients WHERE recipeID="
        + str(recipeID)
    ...
    if ingredientsUpdated > 0:
        ...
        if filledCandidates > 0:
            ...
            returnStr = (
                "That resource cannot be used in any of the slots of that schematic."
            )
    ...
    if errstr == "":
        ...
        if logged_state > 0:
            ...
            if schematic == "":
                ...
                if row != None:
                    if operation == "delete":
                        ...
                        result = addIngredient(
                            conn, recipeID, spawnID, row[1], currentUser
                        )
                    else:
                        result = updateRecipe(conn, recipeID, recipeName)
                        if ingredients != "":
                            result += updateIngredients(
                                conn, recipeID, ingredients, row[1], currentUser
                            )
                else:
                ...
                if ingredients != "":
                    result += updateIngredients(
                        conn, recipeID, ingredients, schematic, currentUser
                    )
            conn.close()
        else:
