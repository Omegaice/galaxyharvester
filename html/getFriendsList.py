...
if len(lastUser) > 2 and len(direction) > 0:
    if direction == "back":
        userCriteria = (
            " AND uf1.friendID < '" + lastUser + "' ORDER BY uf1.friendID DESC"
        )
    else:
...
if cursor and tmpStr == "":
    ...
    if logged_state == 1 and currentUser == uid:
        ...
        sqlStr = (
            'SELECT uf1.friendID, uf2.added, pictureName FROM tUserFriends uf1 INNER JOIN tUsers tu ON uf1.friendID = tu.userID LEFT JOIN tUserFriends uf2 ON uf1.friendID=uf2.userID AND uf1.userID=uf2.friendID WHERE uf1.userID="'
            + uid
            + '"'
            + userCriteria
            + " LIMIT "
        ...
        sqlStr = (
            'SELECT uf1.friendID, uf2.added, pictureName FROM tUserFriends uf1 INNER JOIN tUsers tu ON uf1.friendID = tu.userID INNER JOIN tUserFriends uf2 ON uf1.friendID=uf2.userID AND uf1.userID=uf2.friendID WHERE uf1.userID="'
            + uid
            + '"'
            + userCriteria
            + " LIMIT "
    ...
    while row != None:
        ...
        if logged_state == 1 and currentUser == uid:
            if row[1] == None:
                rowStr += (
                    '<td><img src="/images/eStar16.png" title="'
                    + row[0]
                    + ' has not added you to their friends list yet." /></td>'
                )
            else:
                rowStr += (
                    '<td><img src="/images/rStar16.png" title="You are on '
                    + row[0]
                    + "'s friends list.\" /></td>"
                )
            rowStr += (
                '<td><img src="/images/xGrey16.png" title="Click to remove from friends list." style="cursor: pointer;" onclick="removeFriend(\''
                + row[0]
                + "')\" /></td>"
            )
        rowStr += "  </tr>"
    ...
    if tmpStr != "Error: No Data":
        ...
        if lastUser != "":
            tmpStr += (
                '<button id="prevFriendsButton" class="ghButton" style="margin:10px;" onclick="moreFriends(\''
                + firstFriend
                + "', 'back');\">Back</button>"
            )
        if cursor.rowcount == pageSize:
            tmpStr += (
                '<button id="nextFriendsButton" class="ghButton" style="margin:10px;" onclick="moreFriends(\''
                + lastFriend
                + "', 'next');\">Next</button>"
            )
        tmpStr += "</div>"
