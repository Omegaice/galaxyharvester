...
def getResourceSQL(
    maxCheckStr, favCols, joinStr, galaxy, resGroup, obyStr, obyStr2, mine
):
    sqlStr1 = ""
    ...
    sqlStr1 = (
        ...
        + ") / ("
        + obyStr2
        + ") AS overallScore FROM tResources INNER JOIN tResourceType ON tResources.resourceType = tResourceType.resourceType"
        + joinStr
    )
    ...
    if mine != "":
        sqlStr1 += (
            " AND favGroup IS NOT NULL AND favGroup NOT IN ('Surveying', 'Shopping')"
        )
    if obyStr != "":
        sqlStr1 += " ORDER BY ("
        sqlStr1 += obyStr
...
def getResourceData(
    conn, resSQL, logged_state, galaxyState, resourceFormat, reputation, statsMatter
):
...
if schematicID == "":
    errstr = (
        errstr + "You must specify a schematic to get resource information for. \r\n"
    )
if galaxy == "":
...
if errstr != "":
    ...
    if ingCursor:
        ingSQL = (
            'SELECT res.resID, Max(res.resName) FROM tSchematicIngredients LEFT JOIN (SELECT resourceGroup AS resID, groupName AS resName FROM tResourceGroup UNION ALL SELECT resourceType, resourceTypeName FROM tResourceType) res ON ingredientObject = res.resID WHERE schematicID="'
            + schematicID
            + '" AND ingredientType=0 GROUP BY res.resID;'
        )
        ...
        while ingRow != None:
            ...
            maxCheckStr = ""
            expSQL = (
                'SELECT expGroup, statName, Sum(statWeight) AS sw, Sum(weightTotal) AS wt FROM tSchematicQualities INNER JOIN tSchematicResWeights ON tSchematicQualities.expQualityID = tSchematicResWeights.expQualityID WHERE schematicID="'
                + schematicID
                + '"'
            )
            ...
            if compare == "undefined" or logged_state == 0:
                ...
                resSQL = getResourceSQL(
                    maxCheckStr, favCols, joinStr, galaxy, resGroup, obyStr, obyStr2, ""
                )
                print(
                    getResourceData(
                        conn,
