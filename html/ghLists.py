...
def getResourceTypeList(galaxy="-1"):
    if galaxy != "-1":
        listStr = getOptionList(
            "SELECT resourceType, resourceTypeName FROM tResourceType WHERE (specificPlanet = 0 OR specificPlanet IN (SELECT DISTINCT tPlanet.planetID FROM tPlanet, tGalaxyPlanet WHERE (tPlanet.planetID < 11) OR (tPlanet.planetID = tGalaxyPlanet.planetID AND tGalaxyPlanet.galaxyID = {0})))".format(
                galaxy
            )
        ...
        listStr = getOptionList(
            "SELECT resourceType, resourceTypeName FROM tResourceType ORDER BY resourceTypeName;"
        )
    return listStr
...
def getResourceGroupList():
    listStr = getOptionList(
        "SELECT resourceGroup, groupName FROM tResourceGroup WHERE groupLevel > 1 ORDER BY groupName;"
    )
    return listStr
...
def getResourceGroupListShort():
    listStr = getOptionList(
        "SELECT resourceGroup, groupName FROM tResourceGroup WHERE groupLevel < 4 AND groupLevel > 1 ORDER BY groupName;"
    )
    return listStr
...
def getGalaxyList():
    listStr = getOptionList(
        'SELECT galaxyID, CASE WHEN galaxyNGE > 0 THEN CONCAT(galaxyName, \' [NGE]\') ELSE galaxyName END, CASE WHEN galaxyState=1 THEN "Active" ELSE "Inactive" END FROM tGalaxy WHERE galaxyState > 0 AND galaxyState < 3 ORDER BY galaxyState, galaxyName;'
    )
    return listStr
...
def getPlanetList(galaxy):
    if galaxy == -1 or galaxy == "":
        listStr = getOptionList(
            "SELECT planetID, planetName FROM tPlanet ORDER BY planetName"
        )
    else:
        listStr = getOptionList(
            "SELECT DISTINCT tPlanet.planetID, planetName FROM tPlanet, tGalaxyPlanet WHERE (tPlanet.planetID < 11) OR (tPlanet.planetID = tGalaxyPlanet.planetID AND tGalaxyPlanet.galaxyID = {0}) ORDER BY planetName;".format(
                galaxy
            )
        )
    return listStr
...
def getProfessionList(galaxy):
    if galaxy == -1 or galaxy == "":
        listStr = getOptionList(
            "SELECT profID, profName FROM tProfession WHERE galaxy=0 ORDER BY profName;"
        )
    else:
        listStr = getOptionList(
            "SELECT profID, profName FROM tProfession WHERE galaxy IN ({1},{0}) ORDER BY profName".format(
                str(galaxy), dbShared.getBaseProfs(galaxy)
            )
        )
    return listStr
...
def getObjectTypeList():
    listStr = getOptionList(
        "SELECT objectType, typeName FROM tObjectType ORDER BY typeName;"
    )
    return listStr
