    ...
    while row != None:
        if row[4] == "{0}user.py/{1}".format(ghShared.BASE_SCRIPT_URL, userID):
            ...
            output = (
                ...
                + '<li id="alert_'
                + str(row[0])
                + '"><div class="tableHead"><a href="'
                + row[4]
                + '"><div class="inlineBlock" style="width:90%;">'
                + ghShared.timeAgo(row[2])
                + " ago - "
                + row[3]
                + '</div></a><div class="inlineBlock" style="vertical-align:top;min-width:20px;"><img src="/images/xRed16.png" style="cursor:pointer;" title="Click to remove this alert" alt="Red X" onclick="updateAlertStatus('
                + str(row[0])
                + ', 2)" /></div></div></li>'
            )
        else:
...
if errstr == "":
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT alertID, alertType, alertTime, alertMessage, alertLink, alertStatus FROM tAlerts WHERE userID='"
        + currentUser
        + "' AND alertType IN ("
        + alertTypes
        + ")"
