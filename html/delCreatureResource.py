...
if result == "":
    ...
    if cursor:
        ...
        cursor.execute(
            "SELECT enteredBy FROM tResourceTypeCreature WHERE galaxy=%s AND resourceType=%s AND speciesName=%s;",
            (galaxy, resourceType, creatureName),
        )
        row = cursor.fetchone()
        ...
        if (
            ...
            result = (
                "Error: You do not have permission to edit other users creature data."
            )
