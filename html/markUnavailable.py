...
def removeSpawn(spawnID, planets, userID, galaxy):
    ...
    if cursor:
        ...
        cursor.execute(
            "SELECT enteredBy, unavailable FROM tResources WHERE spawnID=%s;", [spawnID]
        )
        row = cursor.fetchone()
        if (
            int(stats[2]) < ghShared.MIN_REP_VALS["REMOVE_RESOURCE"]
            and row[0] != userID
            and not admin
        ):
            ...
            result = (
                "Error: You cannot remove that resource because it is already removed."
            )
        else:
