                ...
                if row != None:
                    if row[1] == currentUser:
                        ...
                        schematicDetailsHTML = (
                            ...
                            + "schematics.py/"
                            + row[2]
                            + '" title="Go to schematic page.">'
                            + row[5]
                            + "</a></div>"
                        )
                        schematicDetailsHTML += (
                            "<div>Complexity: " + str(row[6]) + "</div>"
                        )
                        expGroup = ""
                        expProp = ""
                        ...
                        while expRow != None:
                            ...
                            if expProp != expRow[1]:
                                ...
                                schematicDetailsHTML = (
                                    schematicDetailsHTML
                                    + '<li class="schemQualityProperty altText">'
                                    + tmpName
                                    + "</li>"
                                )
                                expProp = expRow[1]
                            ...
                            schematicDetailsHTML += (
                                '<li class="schemQualityItem" tag="'
                                + expRow[3]
                                + ":"
                                + str((expRow[4] * 1.0 / expRow[5]) * 100)
                                + '"><span class="inlineBlock" style="width:100px;">'
                                + ghNames.getStatName(expRow[3])
                                + (
                                    ": </span><span>%.0f"
                        ...
                        ri = None
                        sqlStr = (
                            "SELECT si.ingredientName, ingredientResource, ingredientObject, ingredientQuantity, ingredientContribution, rt.containerType tcontainer, rg.containerType gcontainer, rt.resourceTypeName, rg.groupName, ingredientQuality FROM tSchematicIngredients si LEFT JOIN (SELECT ingredientName, ingredientResource, ingredientQuality FROM tRecipeIngredients WHERE recipeID="
                            + str(r.recipeID)
                            + ') ri ON si.ingredientName = ri.ingredientName LEFT JOIN tResourceType rt ON si.ingredientObject = rt.resourceType LEFT JOIN tResourceGroup rg ON si.ingredientObject = rg.resourceGroup WHERE schematicID="'
                            + r.schematicID
                        ...
                        while ingRow != None:
                            if ingRow[5] == None:
                                if ingRow[6] == None:
                                    container = "default"
                                    objectName = (
                                        ingRow[2].rpartition("/")[2].replace("_", " ")
                                    )
                                    if objectName[-4:] == ".iff":
                            ...
                            resDetails = ""
                            if ingRow[1] != None and (
                                ingRow[5] != None or ingRow[6] != None
                            ):
