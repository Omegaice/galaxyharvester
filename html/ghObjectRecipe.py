...
class schematicRecipe:
        ...
        for ing in self.recipeIngredients:
            ...
            if ing.ingredientResource != None:
                ...
                if ing.resourceQuality != None:
                    qualityIndicatorStyle = (
                        ' style="height:'
                        + str(70 * ing.resourceQuality / 1000)
                        + "px;background-color:"
                        + self.getResourceValueColor(ing.resourceQuality)
                        + ';"'
                    )
                addProperties = (
                    ' tag="filled" spawnID="'
                    + str(ing.ingredientResource)
                    + '" tt="'
                    + ing.resourceDetails
                    + '"'
                )
            ...
            # Add slot to result
            result += (
                '<div class="inlineBlock recipeIngredient" tag="'
                + ing.ingredientObject
                + '" title="Requires '
                + str(ing.ingredientObjectName)
                + '">'
            )
            result += (
                '<div class="ingredientHeader">'
                + ing.ingredientName.replace("_", " ")
                + "</div>"
            )
            result += (
                '<div class="ingredientSlot'
                + addClass
                + '" style="background-repeat:no-repeat;background-position: 10px 10px;background-image:url(/images/resources/'
                + str(ing.ingredientType)
                + '.png)"'
                + addProperties
                + ">"
                + amountStr
                + "/"
                + str(ing.ingredientAmount)
                + "</div>"
            )
            result += (
    ...
    def getRow(self, listType="normal", sid=""):
        ...
        qualityIndicatorStyle = (
            ' style="width:'
            + str(140 * qualityAvg / 1000.0)
            + "px;background-color:"
            + self.getResourceValueColor(qualityAvg)
            + ';"'
        )
        ...
        if listType == "suggest":
            result += (
                '  <td style="width:32px;"><img src="/images/schematics/'
                + self.schematicImage
                + '" class="schematicIngredient" /></td>'
            )
            result += (
                '  <td title="'
                + qualityBarInfo
            ...
            result += (
                '  <td style="width:32px;"><a'
                + linkTarget
                + '><img src="/images/schematics/'
                + self.schematicImage
                + '" class="schematicIngredient" /></a></td>'
            )
            result += (
                '  <td title="'
                + qualityBarInfo
                + '"><div><a class="nameLink"'
                + linkTarget
                + ">"
                + self.recipeName
                + "</a></div>"
        ...
        # Quality bar
        result += (
            '  <div class="recipeQualityBar"><div class="qualityIndicator"'
            + qualityIndicatorStyle
            + "></div></div>"
        )
        ...
        for ing in self.recipeIngredients:
            ...
            result += (
                '<div class="inlineBlock slotIndicator'
                + addStyle
                + '" title="'
                + (ing.ingredientName + ": " + ing.ingredientObject).replace("_", " ")
                + addTitle
                + '"></div>'
            )
        result += "</div>"
        ...
        if listType == "suggest":
            ...
            for ing in self.recipeIngredients:
                ingredientString += (
                    ing.ingredientName + ":" + ing.ingredientResource + ","
                )
            result += (
                '  <td><button type=button value="Save" class="ghButton" onclick="saveSuggestedRecipe(\''
                + self.schematicID
                + "','"
                + self.recipeName
                + " suggested','"
                + ingredientString
                + "', $('#galaxySel').val());\">Save</button></td>"
            )
        else:
            result += (
                '  <td><a alt="Delete Recipe" style="cursor: pointer;" onclick="deleteRecipe(this, \''
                + str(self.recipeID)
                + "');\">[X]</a></td>"
            )
