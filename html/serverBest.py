...
def getPosition(conn, spawnID, galaxy, statWeights, resourceGroup, serverBestMode):
    ...
    if serverBestMode == "current":
        minimumPercentOfBest = 0.8
    else:
        minimumPercentOfBest = 0.95
...
def checkSchematics(
    ...
    while ingRow != None:
        ...
        expSQL = (
            'SELECT expGroup, statName, Sum(statWeight)/Sum(weightTotal)*100 AS wp FROM tSchematicQualities INNER JOIN tSchematicResWeights ON tSchematicQualities.expQualityID = tSchematicResWeights.expQualityID WHERE schematicID="'
            + ingRow[0]
            + '" GROUP BY expGroup, statName;'
        )
                ...
                if tmpGroup != "":
                    ...
                    if stats != lastStats:
                        ...
                        if allKey in allPositions:
                            ...
                            spawnPosition = getPosition(
                                conn, spawnID, galaxy, stats, ingRow[1], serverBestMode
                            )
                    ...
                    if spawnPosition > 0:
                        if spawnPosition == 1:
                            eventDetail = "".join(
                                (
                                    "New server best spawn for ",
                                    ingRow[3],
                                    " ",
                                    tmpGroup.replace("exp_", "")
                                    .replace("exp", "")
                                    .replace("_", " "),
                                    ", ingredient ",
                                    ingRow[4],
...
def checkSpawn(spawnID, serverBestMode):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT galaxy, resourceType, (SELECT GROUP_CONCAT(resourceGroup SEPARATOR \"','\") FROM tResourceTypeGroup rtg WHERE rtg.resourceType=tResources.resourceType) FROM tResources WHERE spawnID=%s;",
        [spawnID],
    )
    row = cursor.fetchone()
    if row != None:
        ...
        pc = conn.cursor()
        pc.execute(
            "SELECT profID, profName FROM tProfession WHERE craftingQuality > 0;"
        )
        pcRow = pc.fetchone()
        while pcRow != None:
            result = checkSchematics(
                conn,
                str(spawnID),
                row[0],
                pcRow[0],
                "".join(("'", row[1], "','", str(row[2]), "'")),
                serverBestMode,
                allPositions,
            )
