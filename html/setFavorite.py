...
def favoriteExists(conn, user, favType, item, galaxy):
    ...
    if favType == "2" or favType == "4":
        favSQL = "".join(
            (
                "SELECT itemID FROM tFavorites WHERE favGroup='",
                str(item),
                "' AND userID='",
                user,
                "' AND favType=",
        ...
        favSQL = "".join(
            (
                "SELECT favGroup FROM tFavorites WHERE itemID=",
                str(item),
                " AND userID='",
                user,
                "' AND favType=",
...
def addFavorite(conn, user, favType, item, galaxy):
    ...
    if galaxy.isdigit():
        ...
        if favType == "2" or favType == "4":
            tempSQL = (
                "INSERT INTO tFavorites (userID, favType, itemID, favGroup, galaxy) VALUES ('"
                + user
                + "', "
                + str(favType)
                + ", NULL, '"
...
def removeFavorite(conn, user, favType, item, galaxy):
    ...
    if favType == "2" or favType == "4":
        tempSQL = "".join(
            (
                "DELETE FROM tFavorites WHERE favGroup='",
                str(item),
                "' AND userID='",
                user,
                "' AND favType=",
        ...
        tempSQL = "".join(
            (
                "DELETE FROM tFavorites WHERE itemID=",
                str(item),
                " AND userID='",
                user,
                "' AND favType=",
...
if not re.match("^[\w _]+$", favGroup) and favGroup != "":
    errstr = (
        errstr
        + "Error: group name contains illegal characters (only alpha numeric and spaces or underscores allowed)."
    )
if len(favGroup) > 255:
...
if units != "" and units.isdigit() == False:
    errstr = (
        errstr
        + "Error: The value you entered for units contains non-numeric characters."
    )
if units != "" and units.isdigit() == True:
    try:
        ...
        if unitsTest > 2147483648 or unitsTest < -2147483648:
            errstr = (
                errstr + "Error: That value for units is outside the range I can store."
            )
    except:
        errstr = (
            errstr
            + "Error: The value you entered for units could not be converted to a number."
        )
...
if errstr == "":
    ...
    if logged_state > 0:
        ...
        if favType == "1" and itemID.isdigit() == False:
            ...
            if itemID == "-1":
                result = (
                    "Error: The spawn name you entered cannot be found in this galaxy."
                )
            ...
            if units != "" or favGroup != "" or despawnAlert != "":
                ...
                if favoriteExists(conn, currentUser, favType, itemID, galaxyID):
                    if operation != "1":
                        ...
                        remresult = removeFavorite(
                            conn, currentUser, favType, itemID, galaxyID
                        )
                        if remresult > 0:
                        ...
                        result = (
                            "Error: You already have that resource set as a favorite."
                        )
                else:
                    # does not exist, adding as new favorite
                    addresult = addFavorite(
                        conn, currentUser, favType, itemID, galaxyID
                    )
                    if (
                        str(result).find("Error:") == -1
                        and addresult.isdigit()
                        and addresult > "0"
                    ):
