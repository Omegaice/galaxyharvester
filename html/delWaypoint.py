...
if logged_state > 0:
    ...
    if cursor:
        # check owner
        cursor.execute(
            "SELECT owner FROM tWaypoint WHERE waypointID=" + str(wpID) + ";"
        )
        row = cursor.fetchone()
        ...
        if owner == currentUser:
            sqlStr = (
                "UPDATE tWaypoint SET unavailable=NOW() WHERE waypointID="
                + str(wpID)
                + ";"
            )
            ...
            sqlStr = (
                "UPDATE tUserWaypoints SET unlocked=NULL WHERE userID='"
                + currentUser
                + "' AND waypointID="
                + str(wpID)
                + ";"
            ...
            if affRows > 0:
                ...
                cursor.execute(
                    "SELECT shareLevel FROM tWaypoint WHERE waypointID="
                    + str(wpID)
                    + ";"
                )
                row = cursor.fetchone()
                ...
                if shareLevel == 256:
                    ...
                    if row == None:
                        ...
                        cursor.execute(
                            "SELECT Sum(repGood)-Sum(repBad) FROM tUserStats WHERE userID IN (SELECT ue.userID FROM tUserEvents ue WHERE targetType='w' AND targetID="
                            + str(wpID)
                            + " AND eventType='r');"
                        )
                        row = cursor.fetchone()
