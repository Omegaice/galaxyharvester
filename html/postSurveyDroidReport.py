...
def updatePlanetSpawns(planetID, classID, resources, galaxyID, userID):
    ...
    for spawn in resources:
        ...
        if row != None:
            if row[0] != None:
                ...
                if (timeSinceRemoved.days < 3 and timeSinceRemoved.days > 0) or (
                    timeSinceRemoved.days == 0 and timeSinceRemoved.seconds > 7200
                ):
                    status = postResource.addResPlanet(
                        row[1], planetID, spawn.spawnName, userID, galaxyID
                    )
            ...
            typecursor.execute(
                "SELECT resourceTypeName FROM tResourceType WHERE resourceType=%s",
                [spawn.resourceType],
            )
            typerow = typecursor.fetchone()
            if typerow != None:
                status = postResource.addResource(
                    spawn.spawnName,
                    galaxyID,
                    spawn.resourceType,
                    str(spawn.stats.CR),
                ...
                if status.find("Error:") == -1:
                    ...
                    status = (
                        postResource.addResPlanet(
                            spawnID, planetID, spawn.spawnName, userID, galaxyID
                        )
                        + "   "
                        + status
                    )
    ...
    result = "Report Upload Complete...\n<br/>{0} Errors\n<br/>{1} Spawns Added\n<br/>{2} Spawns Verified\n<br/>{3} Spawns Removed.\n<br/>{4}".format(
        spawnErrorCount, spawnAddCount, spawnVerifyCount, spawnRemovedCount, errors
    )
    return result
...
def main():
    ...
    if "reportFile" not in form:
        errstr = "No report file sent."
    else:
        rpt_data = form["reportFile"]
        if not rpt_data.file:
            errstr = "report data is not a file."
    ...
    if errstr != "":
        result = (
            "Your report could not be posted because of the following errors:\r\n"
            + errstr
        )
    else:
        result = ""
        ...
        while 1:
            ...
            if classMatch == None:
                classMatch = re.match(
                    "\\\#pcontrast\d\sResource Class: \\\#pcontrast\d\s(.+)", line
                )
            ...
            if resourceStart == None:
                ...
                if statMatch:
                    setattr(
                        thisSpawn.stats, statMatch.group(1), int(statMatch.group(2))
                    )
        ...
        if planetID > "0":
            if dataAction == "returnJSON":
                result = getSpawnsJSON(
                    planetID,
                    resourcesFound,
                    "{0} spawns loaded from report".format(len(resourcesFound)),
                )
            else:
                result = updatePlanetSpawns(
                    planetID, classID, resourcesFound, galaxyID, currentUser
                )
        else:
