...
if planetID.isdigit() and int(planetID) > 0:
    criteriaStr = (
        criteriaStr
        + " AND (specificPlanet = 0 OR specificPlanet = {0})".format(planetID)
    )
else:
    if galaxy.isdigit() and int(galaxy) > 0:
        criteriaStr = (
            criteriaStr
            + " AND (specificPlanet = 0 OR specificPlanet IN (SELECT DISTINCT tPlanet.planetID FROM tPlanet, tGalaxyPlanet WHERE (tPlanet.planetID < 11) OR (tPlanet.planetID = tGalaxyPlanet.planetID AND tGalaxyPlanet.galaxyID = {0})))".format(
                galaxy
            )
        )
...
if cursor:
    ...
    if row == None and len(resGroup) > 0:
        cursor.execute(
            'select rgc.resourceGroup, rg.groupName, "p11111111111" AS statMask, containerType FROM tResourceGroupCategory rgc INNER JOIN tResourceGroup rg ON rgc.resourceGroup = rg.resourceGroup WHERE rgc.resourceCategory="'
            + resGroup
            + '";'
        )
        row = cursor.fetchone()
