...
def refreshServerBestStatus(spawnID):
    ...
    result = ""
    sqlStr = "SELECT eventTime FROM tServerBestStatus WHERE spawnID={0};".format(
        spawnID
    )
    ...
    if checkRow != None:
        ...
        if hoursAgo > 1:
            ...
            cleanCursor.execute(
                "DELETE FROM tServerBestStatus WHERE spawnID=%s;", [spawnID]
            )
            cleanCursor.close()
...
if cursor and errors == "":
    if spawnID.isdigit():
        ...
        criteriaStr = ' WHERE spawnID=0 AND tSchematicEvents.schematicID="{0}"'.format(
            schematicID
        )
    ...
    if spawnID.isdigit():
        ...
        cursor.execute(
            "SELECT profName, {1}.schematicID, schematicName, expGroup, eventType FROM {1} INNER JOIN tSchematic ON {1}.schematicID = tSchematic.schematicID INNER JOIN tSkillGroup ON tSchematic.skillGroup = tSkillGroup.skillGroup INNER JOIN tProfession ON tSkillGroup.profID = tProfession.profID{0} ORDER BY profName, schematicName, expGroup".format(
                criteriaStr, eventTable
            )
        )
    else:
        cursor.execute(
            "SELECT eventTime, expGroup, eventType, eventDetail FROM tSchematicEvents{0} ORDER BY eventTime".format(
                criteriaStr
            )
        )
            ...
            if spawnID.isdigit():
                ...
                responseData = "".join(
                    (
                        ...
                        row[2],
                        "</a></td><td>",
                        row[3].replace("exp_", "").replace("exp", "").replace("_", " "),
                        "</td><td>",
                        rankStr,
                        "</td>",
                    )
                ...
                responseData = "".join(
                    (
                        ...
                        row[1],
                        "</a></td><td>",
                        ghShared.SCHEMATIC_EVENT_NAMES[row[2]],
                        "</td><td>",
                        row[3],
                        "</td>",
                    )
