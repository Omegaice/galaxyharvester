...
def addWaypoint(
    spawnID, planetID, price, concentration, lattitude, longitude, wpName, shareLevel
):
    # Add new waypoint
    returnStr = ""
    ...
    cursor = conn.cursor()
    tempSQL = (
        "INSERT INTO tWaypoint (spawnID, planetID, owner, price, concentration, lattitude, longitude, waypointType, waypointName, shareLevel, entered) VALUES ("
        + str(spawnID)
        + ","
        + str(planetID)
        + ",'"
...
if location.find(",") > -1:
    ...
    errstr = (
        errstr
        + "Error: location is not in the right format. Separate the cooridinates with a comma."
    )
    ...
    if logged_state > 0:
        ...
        if spawnID > -1 or forceOp == "verify":
            ...
            if wpID != None and wpID.isdigit():
                ...
                if cursor:
                    cursor.execute(
                        "SELECT owner FROM tWaypoint WHERE waypointID="
                        + str(wpID)
                        + ";"
                    )
                    row = cursor.fetchone()
                    ...
                    if forceOp == "verify":
                        if galaxy.isdigit():
                            if owner != currentUser:
                                dbShared.logUserEvent(
                                    currentUser, galaxy, "w", wpID, "v"
                                )
                                result = "Verified!"
                            else:
                ...
                if cursor:
                    if int(shareLevel) == 256:
                        cursor.execute(
                            "SELECT owner FROM tWaypoint WHERE spawnID="
                            + str(spawnID)
                            + " AND planetID="
                            + str(planet)
                            + " AND shareLevel=256 AND lattitude BETWEEN "
                            + str(lattitude - 50)
                            + " AND "
                            + str(lattitude + 50)
                    ...
                    if owner == "":
                        ...
                        result = (
                            "Error: That public waypoint has already been entered by "
                            + owner
                        )
                else:
                    result = "Error: No database connection"
            ...
            result = (
                "Error: I can not find the resource name you entered in this galaxy."
            )
