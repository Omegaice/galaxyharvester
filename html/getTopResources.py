...
def getProfOrder(profID):
    ...
    if profID.isdigit():
        ...
        expCursor.execute(
            'SELECT "holdername", statName, Sum(statWeight) AS sw, Sum(weightTotal) AS wt FROM tSchematicQualities INNER JOIN tSchematicResWeights ON tSchematicQualities.expQualityID = tSchematicResWeights.expQualityID INNER JOIN tSchematic ON tSchematicQualities.schematicID = tSchematic.schematicID INNER JOIN tSkillGroup ON tSchematic.skillGroup = tSkillGroup.skillGroup WHERE profID='
            + prof
            + " GROUP BY statName;"
        )
...
def getTypeOrder(tabID, typeID):
    ...
    if tabID.isdigit():
        sqlStr = (
            'SELECT "holdername", statName, Sum(statWeight) AS sw, Sum(weightTotal) AS wt FROM tSchematicQualities INNER JOIN tSchematicResWeights ON tSchematicQualities.expQualityID = tSchematicResWeights.expQualityID INNER JOIN tSchematic ON tSchematicQualities.schematicID = tSchematic.schematicID WHERE craftingTab='
            + tabID
        )
        if typeID.isdigit():
...
def getTypeResGroups(tabID, typeID):
    ...
    if tabID.isdigit():
        sqlStr = (
            "(SELECT rtg.resourceGroup FROM tResourceTypeGroup rtg INNER JOIN tSchematicIngredients ON rtg.resourceType = tSchematicIngredients.ingredientObject INNER JOIN tSchematic ON tSchematicIngredients.schematicID = tSchematic.schematicID WHERE tSchematic.craftingTab="
            + tabID
        )
        if typeID.isdigit():
        ...
        sqlStr = (
            sqlStr
            + " GROUP BY rtg.resourceGroup) UNION (SELECT rgc.resourceGroup FROM tResourceGroupCategory rgc INNER JOIN tSchematicIngredients ON rgc.resourceCategory = tSchematicIngredients.ingredientObject INNER JOIN tSchematic ON tSchematicIngredients.schematicID = tSchematic.schematicID WHERE tSchematic.craftingTab="
            + tabID
        )
        if typeID.isdigit():
            sqlStr = sqlStr + " AND tSchematic.objectType=" + typeID
        sqlStr = (
            sqlStr
            + " GROUP BY rgc.resourceGroup) UNION (SELECT rgc.resourceGroup FROM tResourceGroupCategory rgc INNER JOIN tSchematicIngredients ON rgc.resourceGroup = tSchematicIngredients.ingredientObject INNER JOIN tSchematic ON tSchematicIngredients.schematicID = tSchematic.schematicID WHERE tSchematic.craftingTab="
            + tabID
        )
        if typeID.isdigit():
            sqlStr = sqlStr + " AND tSchematic.objectType=" + typeID
...
if galaxy != "":
    ...
    if cursor:
        ...
        sqlStr1 += (
            " containerType, "
            + orderVals
            + ", verified, verifiedBy, unavailable, unavailableBy"
            + favCols
            + " FROM tResources INNER JOIN tResourceType ON tResources.resourceType = tResourceType.resourceType"
            + favJoin
        )
        if resGroup != "any" and resGroup != "":
            sqlStr1 += (
                ...
                + resGroup
                + '" GROUP BY resourceType) rtg ON tResources.resourceType = rtg.resourceType'
            )
        if unavailable == "1":
        ...
        if prof.isdigit() and prof != "0":
            sqlStr1 += (
                " AND tResourceType.resourceGroup IN (" + getProfResGroups(prof) + ")"
            )
        if craftingTab.isdigit():
            sqlStr1 += (
                " AND tResourceType.resourceGroup IN ("
                + getTypeResGroups(craftingTab, objectType)
                + ")"
            )
        sqlStr1 += " ORDER BY"
        sqlStr1 += orderVals
