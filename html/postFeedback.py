...
if re.search("[><&]", suggestion):
    errstr = (
        errstr + "There are illegal characters in your suggestion.  No HTML allowed."
    )
...
if len(errstr) < 5:
    ...
    if suggestion == "":
        cursor = cconn.cursor()
        cursor.execute(
            "SELECT vote FROM tFeedbackVotes WHERE feedbackID="
            + str(itemID)
            + " AND userID='"
            + currentUser
            + "';"
        ...
        if row != None:
            if str(row[0]) != voteValue:
                ccursor.execute(
                    "UPDATE tFeedbackVotes SET vote="
                    + str(voteValue)
                ...
                ccursor.execute(
                    "UPDATE tFeedbackVotes SET vote=0, entered=NOW() WHERE feedbackID="
                    + str(itemID)
                    + " AND userID='"
                    + currentUser
                    + "';"
            ...
            ccursor.execute(
                "INSERT INTO tFeedbackVotes (feedbackID, entered, userID, vote) VALUES ("
                + str(itemID)
                + ",NOW(),'"
                + currentUser
                + "',"
        ...
        cursor = cconn.cursor()
        cursor.execute(
            "SELECT entered FROM tFeedback WHERE feedbackID=" + str(itemID) + ";"
        )
        row = cursor.fetchone()
        ccursor = cconn.cursor()
        if row != None:
            addType = "Comment"
            ccursor.execute(
                "INSERT INTO tFeedbackComments (feedbackID, entered, userID, comment) VALUES ("
                + str(itemID)
                + ",NOW(),'"
                + currentUser
                + "','"
            ...
            ccursor.execute(
                "INSERT INTO tFeedback (entered, userID, feedback, feedbackState) VALUES (NOW(),'"
                + currentUser
                + "','"
                + suggestion
                + "',1);"
    ...
    result = (
        "Error: Feedback could not be updated because of the following errors:\r\n"
        + errstr
    )
