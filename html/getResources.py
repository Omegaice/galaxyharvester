...
def getResourceSQL(
    ...
    sqlStr1 += (
        " rt1.containerType, tResources.verified, tResources.verifiedBy, tResources.unavailable, tResources.unavailableBy, rg1.groupName, rt1.resourceCategory, rg2.groupName AS categoryName, rt1.resourceGroup, (SELECT Max(concentration) FROM tWaypoint WHERE tWaypoint.spawnID=tResources.spawnID AND tWaypoint.unavailable IS NULL AND ("
        + wpCriteria
        + ")) AS wpMaxConc"
        + favCols
        + ", "
    ...
    if cursor:
        ...
        if fetchSize.isdigit() and cursor.rowcount == int(fetchSize):
            if formatType == "json":
                ...
                resourceHTML += (
                    '<div style="display:none;">maxRowsReached'
                    + str(lastValue)
                    + "</div>"
                )
...
if planet != "any" and planet != "" and planet != "0":
    joinStr = (
        joinStr
        + " INNER JOIN tResourcePlanet ON tResources.spawnID = tResourcePlanet.spawnID"
    )
...
if logged_state == 1:
    ...
    if favorite == "on":
        joinStr = (
            joinStr
            + " INNER JOIN (SELECT itemID, favGroup, despawnAlert, units FROM tFavorites WHERE userID='"
            + currentUser
        ...
        joinStr = (
            joinStr
            + ' LEFT JOIN (SELECT itemID, favGroup, despawnAlert, units FROM tFavorites WHERE userID="'
            + currentUser
            + '" AND favType=1) favs ON tResources.spawnID = favs.itemID'
        )
...
if errorStr == "":
    ...
    if compare == "" or compare == "undefined" or logged_state == 0:
        ...
        if tokenPosition > -1:
            resData += (
                '<div style="text-align:center;"><button id="nextResourcesButton" class="ghButton" style="margin:10px;" onclick="moreResources(\''
                + resData[tokenPosition + 14 : resData.find("</div>", tokenPosition)]
                + "', 'next');\">Next 20</button></div>"
            )
    else:
        ...
        resSQL = getResourceSQL(
            wpCriteria, favCols, joinStr, orderCol, orderStr, criteriaStr, "", "y"
        )
        resData += getResourceData(conn, resSQL, userReputation, activeUser, formatType)
        resData += "</div></div>"
        tokenPosition = resData.find("maxRowsReached")
    ...
    if formatType == "json":
        responseData = "{\n"
        resCountSQL = getResourceCountSQL(
            wpCriteria,
            favCols,
            joinStr,
            orderCol,
            ...
            if row == None or row[0] == 0:
                ...
                if fetchSize.isdigit():
                    responseData += (
                        ...
                        + str(row[0])
                        + ',\n "total_pages" : '
                        + str("%.0f" % (float((row[0] / int(fetchSize)) + 1)))
                        + ",\n"
                    )
                else:
                    responseData += (
