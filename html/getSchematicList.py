    ...
    if resGroup != "" and resGroup != None:
        if resSecondary == "1":
            joinStr = (
                ...
                + resGroup
                + '") rtg ON ingredientObject = rtg.resourceType'
            )
        else:
        ...
        if resSecondary == "1":
            filterStr = (
                ' WHERE ingredientObject IN (SELECT tResourceTypeGroup.resourceGroup FROM tResourceTypeGroup INNER JOIN tResourceGroup ON tResourceTypeGroup.resourceGroup=tResourceGroup.resourceGroup WHERE resourceType="'
                + resType
                + '" AND groupLevel=(SELECT Max(rg.groupLevel) FROM tResourceTypeGroup rtg INNER JOIN tResourceGroup rg ON rtg.resourceGroup = rg.resourceGroup WHERE rtg.resourceType="'
                + resType
                + '") GROUP BY tResourceTypeGroup.resourceGroup)'
            )
            joinStr = (
                ' INNER JOIN (SELECT tResourceTypeGroup.resourceGroup FROM tResourceTypeGroup INNER JOIN tResourceGroup ON tResourceTypeGroup.resourceGroup=tResourceGroup.resourceGroup WHERE resourceType="'
                + resType
                + '" AND groupLevel=(SELECT Max(rg.groupLevel) FROM tResourceTypeGroup rtg INNER JOIN tResourceGroup rg ON rtg.resourceGroup = rg.resourceGroup WHERE rtg.resourceType="'
                + resType
                + '") GROUP BY tResourceTypeGroup.resourceGroup) rtgg ON ingredientObject = rtgg.resourceGroup'
            )
        else:
...
if cursor:
    if groupType == "tab" or groupType == "favorite":
        sqlStr1 = (
            "SELECT schematicID, tSchematic.craftingTab, typeName, schematicName FROM tSchematic INNER JOIN tObjectType ON tSchematic.objectType = tObjectType.objectType"
            + joinStr
            + filterStr
            + " ORDER BY craftingTab, typeName, schematicName"
        )
    elif groupType == "res":
        sqlStr1 = (
            "SELECT DISTINCT tSchematic.schematicID, tSchematic.craftingTab, typeName, schematicName, ingredientObject, res.resName FROM tSchematic INNER JOIN tObjectType ON tSchematic.objectType = tObjectType.objectType INNER JOIN tSchematicIngredients ON tSchematic.schematicID = tSchematicIngredients.schematicID"
            + joinStr
            + " LEFT JOIN (SELECT resourceGroup AS resID, groupName AS resName FROM tResourceGroup UNION ALL SELECT resourceType, resourceTypeName FROM tResourceType) res ON ingredientObject = res.resID"
            + filterStr
            + " ORDER BY res.resName, craftingTab, typeName, schematicName"
        )
    else:
        sqlStr1 = (
            "SELECT schematicID, profName, skillGroupName, schematicName FROM tSchematic INNER JOIN tSkillGroup ON tSchematic.skillGroup = tSkillGroup.skillGroup LEFT JOIN tProfession ON tSkillGroup.profID = tProfession.profID"
            + joinStr
            + filterStr
        )
        if listFormat == "option":
        ...
        if listFormat == "option":
            ...
            if groupType == "res":
                if currentIngredient != row[5]:
                    print("  </ul>")
                    print(
                        '  <div  style="margin-top:14px;"><a class="bigLink" href="'
