...
def buildBestRecipe(conn, schem, inventory, user):
    ...
    oingSql = (
        'SELECT ingredientObject, Sum(ingredientContribution) FROM tSchematicIngredients WHERE schematicID="'
        + schem[0]
        + '" AND ingredientType = 0 GROUP BY ingredientObject;'
    )
    ...
    while oingRow != None and oingRow[0] != None:
        ...
        for resType in inventory.keys():
            if str(resType).find("'" + oingRow[0] + "'") > -1:
                ...
                for s in spawns:
                    spawnQuality = recipe.calcIngredientQuality(
                        conn, s.split("|")[0].strip(","), schem[0], user
                    )
    ...
    if filledIng:
        ...
        cingSql = (
            'SELECT si.ingredientName, ingredientObject, ingredientQuantity, ingredientContribution, rt.containerType tcontainer, rg.containerType gcontainer, rt.resourceTypeName, rg.groupName FROM tSchematicIngredients si LEFT JOIN tResourceType rt ON si.ingredientObject = rt.resourceType LEFT JOIN tResourceGroup rg ON si.ingredientObject = rg.resourceGroup WHERE schematicID="'
            + r.schematicID
            + '" ORDER BY ingredientQuantity DESC, si.ingredientName'
        )
            ...
            for ing in ingredients:
                if ing[0] == cingRow[1]:
                    ...
                    spawn = resource.getResource(
                        conn, 1, user, ing[1].split("|")[0].strip(","), None, None
                    )
                    resDetails = (
                        "Loaded with: "
                        + spawn.spawnName
                        + ", "
                        + spawn.resourceTypeName
                        + "<br />"
                        + spawn.getStatList()
                    )
...
def getSuggestedRecipes(conn, user, galaxy, prof):
    ...
    if cursor:
        ...
        sqlStr1 += (
            ' units, (SELECT GROUP_CONCAT(resourceGroup SEPARATOR "\',\'") FROM tResourceTypeGroup rtg WHERE rtg.resourceType=tResources.resourceType) FROM tResources INNER JOIN tFavorites ON tResources.spawnID = tFavorites.itemID WHERE userID="'
            + user
            + '" AND favType=1 AND tResources.galaxy='
            + galaxy
        ...
        row = cursor.fetchone()
        while row != None:
            inventory["'" + str(row[2]) + "','" + str(row[15]) + "'"] = (
                inventory.get("'" + str(row[2]) + "'," + str(row[15]), "")
                + ","
                + str(row[0])
                + "|CR"
                + str(row[3])
    ...
    for resType in inventory.keys():
        ...
        sqlStr2 = (
            "SELECT tSchematic.schematicID, ingredientObject, Sum(ingredientContribution), schematicName, (SELECT imageName FROM tSchematicImages tsi WHERE tsi.schematicID=tSchematic.schematicID AND tsi.imageType=1) AS schemImage FROM tSchematicIngredients INNER JOIN tSchematic ON tSchematicIngredients.schematicID = tSchematic.schematicID INNER JOIN tSkillGroup ON tSchematic.skillGroup = tSkillGroup.skillGroup WHERE profID="
            + prof
            + " AND ingredientObject IN ("
            + str(resType)
            + ") GROUP BY tSchematic.schematicID, ingredientObject, ingredientQuantity, ingredientName ORDER BY tSchematic.schematicID, ingredientQuantity DESC, ingredientName;"
        )
        ...
        while ingRow != None:
            for s in spawns:
                spawnQuality = recipe.calcIngredientQuality(
                    conn, s.split("|")[0].strip(","), ingRow[0], user
                )
                # Add to list of top quality spawn to ingredient with schemID
                if spawnQuality is not None:
                    schems.append(
                        (ingRow[0], ingRow[1], s, spawnQuality, ingRow[3], ingRow[4])
                    )
            ingRow = ingCursor.fetchone()
        ingCursor.close()
...
def main():
    ...
    if cursor and errstr == "":
        ...
        if listType == "suggest":
            ...
            sqlStr = (
                'SELECT recipeID, userID, tRecipe.schematicID, recipeName, (SELECT imageName FROM tSchematicImages img WHERE img.schematicID=tRecipe.schematicID AND img.imageType=1) AS schemImage FROM tRecipe INNER JOIN tSchematic ON tRecipe.schematicID = tSchematic.schematicID LEFT JOIN tSkillGroup ON tSchematic.skillGroup = tSkillGroup.skillGroup WHERE userID="'
                + currentUser
                + '" AND (tRecipe.galaxy='
                + str(galaxy)
                + " OR tRecipe.galaxy IS NULL)"
                + profFilter
                + " ORDER BY recipeName;"
            )
            ...
            while row != None:
                ...
                sqlStr = (
                    "SELECT si.ingredientName, ingredientResource, ingredientObject, ingredientContribution, ingredientQuality FROM tSchematicIngredients si LEFT JOIN (SELECT ingredientName, ingredientResource, ingredientQuality FROM tRecipeIngredients WHERE recipeID="
                    + str(r.recipeID)
                    + ') ri ON si.ingredientName = ri.ingredientName WHERE schematicID="'
                    + r.schematicID
                    + '" and ingredientType = 0 ORDER BY ingredientQuantity DESC, si.ingredientName;'
                )
                ingCursor = conn.cursor()
