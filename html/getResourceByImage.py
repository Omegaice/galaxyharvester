...
try:
    ...
    errstr = (
        errstr
        + "Error: I don't recognize the file you uploaded as an image ("
        + imgName
...
if errstr != "":
    result = (
        "Error: Could not detect resource because of the following errors:\r\n" + errstr
    )
else:
    result = ""
    ...
    if result == "":
        ...
        ocrOutput = ""
        try:
            ocrOutput = subprocess.check_output(
                ["tesseract", "temp/" + imageName + ".png", "temp/" + imageName]
            )
        ...
        if result == "":
            ...
            for line in f:
                ...
                if ":" in adjLine:
                    vp = adjLine.split(":")
                    if (
                        len(difflib.get_close_matches(vp[0], ["Resource Type"], 1, 0.7))
                        > 0
                    ):
                        s.spawnName = vp[1].strip()
                    if (
                        len(
                            difflib.get_close_matches(vp[0], ["Resource Class"], 1, 0.7)
                        )
                        > 0
                    ):
                    ...
                    if (
                        len(
                            difflib.get_close_matches(
                                vp[0], ["Entangle Resistance"], 1, 0.8
                            )
                        )
                        > 0
                    ):
                    ...
                    if (
                        ...
                        try:
                            ...
                            pass
                    if (
                        len(difflib.get_close_matches(vp[0], ["Conductivity"], 1, 0.8))
                        > 0
                    ):
                        try:
                    ...
                    if (
                        ...
                        try:
                            ...
                            pass
                    if (
                        len(difflib.get_close_matches(vp[0], ["Malleability"], 1, 0.8))
                        > 0
                    ):
                        try:
                    ...
                    if (
                        len(
                            difflib.get_close_matches(vp[0], ["Unit Toughness"], 1, 0.8)
                        )
                        > 0
                    ):
                        try:
