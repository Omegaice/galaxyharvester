...
def deleteSchematic(conn, schematicID):
    ...
    qualCursor.execute(
        "SELECT expQualityID FROM tSchematicQualities WHERE schematicID=%s;",
        (schematicID),
    )
    qualRow = qualCursor.fetchone()
    while qualRow != None:
        delCursor.execute(
            "DELETE FROM tSchematicResWeights WHERE expQualityID=%s;", (qualRow[0])
        )
        qualRow = qualCursor.fetchone()
    qualCursor.close()
    delCursor.execute(
        "DELETE FROM tSchematicQualities WHERE schematicID=%s;", (schematicID)
    )
    # Remove ingredients and main schematic record.
    delCursor.execute(
        "DELETE FROM tSchematicIngredients WHERE schematicID=%s;", (schematicID)
    )
...
if logged_state > 0:
    ...
    if cursor:
        # check owner
        cursor.execute(
            "SELECT enteredBy, galaxy, schematicName FROM tSchematic WHERE schematicID=%s;",
            (schematicID),
        )
        row = cursor.fetchone()
        cursor.close()
        ...
        if owner == currentUser:
            ...
            if galaxy != 0:
                if (
                    admin
                    or userReputation >= ghShared.MIN_REP_VALS["EDIT_OTHER_SCHEMATIC"]
                ):
                    result = deleteSchematic(conn, schematicID)
                else:
                ...
                if (
                    ...
                    cursor = conn.cursor()
                    cursor.execute(
                        "INSERT INTO tSchematicOverrides (schematicID, galaxyID) VALUES (%s, %s);",
                        [schematicID, userGalaxy],
                    )
        ...
        if result.find("Error:") < 0:
            dbShared.logSchematicEvent(
                ...
                currentUser,
                "d",
                "Deleted custom schematic {0}.".format(schematicName),
                "history",
            )
    else:
