...
if errstr != "":
    result = (
        "Your other info could not be updated because of the following errors:\r\n"
        + errstr
    )
else:
    ...
    cursor = conn.cursor()
    cursor.execute(
        "UPDATE tUsers SET themeName='{0}', inGameInfo='{1}', defaultAlertTypes={2}, sharedInventory={3}, sharedRecipes={4} WHERE userID='{5}';".format(
            uiTheme,
            inGameInfo,
            defaultAlerts,
            sharedInventory,
