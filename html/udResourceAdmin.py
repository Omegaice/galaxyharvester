...
if errstr != "":
    result = (
        "Error: Resource admin operation failed because of the following errors:\r\n"
        + errstr
    )
else:
    ...
    checkCursor.execute(
        "SELECT spawnName, galaxy, galaxyName FROM tResources INNER JOIN tGalaxy ON tResources.galaxy = tGalaxy.galaxyID WHERE spawnID=%s",
        (spawnID),
    )
    ...
    if checkRow != None:
        ...
        if finalName != newSpawnName:
            ...
            checkCursor2.execute(
                ...
                (newSpawnName, finalGalaxy),
            )
            checkRow2 = checkCursor2.fetchone()
            if checkRow2 == None:
                fixCursor.execute(
                    "UPDATE tResources SET spawnName=%s WHERE spawnID=%s;",
                    (newSpawnName, spawnID),
                )
            ...
            if checkRow2 == None:
                ...
                if fixCursor.rowcount > 0:
                    ...
                    dbShared.logEvent(
                        ...
                        + checkRow[2]
                        + " to "
                        + ghNames.getGalaxyName(newGalaxyId)
                        + "');",
                        "g",
                        currentUser,
                        finalGalaxy,
        ...
        if updated != "nothing":
            newLink = (
                ": <a href='{0}resource.py/{1}/{2}'>Go to Updated Resource</a>".format(
