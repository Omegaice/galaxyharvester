...
def getResourceHistory(conn, spawnID):
    ...
    resHistory = ""
    sqlStr = (
        'SELECT eventTime, userID, eventType, planetName, eventDetail, galaxyName FROM tResourceEvents INNER JOIN tResources ON tResourceEvents.spawnID = tResources.spawnID LEFT JOIN tPlanet ON tResourceEvents.planetID = tPlanet.planetID LEFT JOIN tGalaxy ON tResourceEvents.galaxy = tGalaxy.galaxyID WHERE tResources.spawnID="'
        + str(spawnID)
    ...
    if cursor:
        ...
        while row != None:
            ...
            resHistory += '  <tr class="statRow"><td>{0}</td><td>{1}</td><td>{2}</td><td>{3}</td><td>{4}</td><td>{5}</td>'.format(
                str(row[0]),
                row[1],
                ghShared.getActionName(row[2]),
                str(row[3]),
                details,
                row[5],
            )
...
def getResource(conn, logged_state, currentUser, spawnID, galaxy, spawnName):
    ...
    if cursor:
        ...
        sqlStr += (
            " rt1.containerType, verified, verifiedBy, unavailable, unavailableBy, rg1.groupName, rt1.resourceCategory, rg2.groupName AS categoryName"
            + favCols
            + ', (SELECT GROUP_CONCAT(resourceGroup SEPARATOR ",") FROM tResourceTypeGroup rtg WHERE rtg.resourceType=tResources.resourceType) FROM tResources INNER JOIN tResourceType rt1 ON tResources.resourceType = rt1.resourceType INNER JOIN tResourceGroup rg1 ON rt1.resourceGroup = rg1.resourceGroup INNER JOIN tResourceGroup rg2 ON rt1.resourceCategory = rg2.resourceGroup'
            + favJoin
            + " WHERE "
            + criteriaStr
            + ";"
...
def main():
    ...
    if len(path) > 1:
        ...
        if galaxy != "":
            ...
            spawn = getResource(
                conn, logged_state, currentUser, None, galaxy, spawnName
            )
            if spawn != None:
    ...
    print(
        template.render(
            ...
            showAdmin=(
                userReputation >= ghShared.MIN_REP_VALS["EDIT_RESOURCE_GALAXY_NAME"]
                or admin
            ),
            spawnID=spawnID,
            spawnGalaxy=galaxy,
            spawnStats=spawnStats,
            spawnContainerType=spawnContainerType,
            spawnResourceTypeName=spawnResourceTypeName,
