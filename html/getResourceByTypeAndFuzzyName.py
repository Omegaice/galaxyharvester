...
if cursor:
    sqlStr = (
        "SELECT spawnID, spawnName, CR, CD, DR, FL, HR, MA, PE, OQ, SR, UT, ER FROM tResources WHERE galaxy="
        + galaxy
        + ' AND resourceType="'
        + resType
    ...
    while row != None:
        ...
        if len(difflib.get_close_matches(spawnName, [row[1]], 1, 0.7)) > 0:
            ...
            pcursor = conn.cursor()
            pcursor.execute(
                'SELECT tResourcePlanet.planetID, planetName FROM tResourcePlanet INNER JOIN tPlanet ON tResourcePlanet.planetID=tPlanet.planetID WHERE spawnID="'
                + spawnID
                + '";'
            )
