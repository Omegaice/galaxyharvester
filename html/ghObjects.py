...
class resourceSpawn:
    ...
    def getPlanetBar(self):
        ...
        for planet in self.planets:
            ...
            if planet.enteredBy != None:
                ...
                result += '"'
                availability = (
                    planet.planetName + " - marked available by " + planet.enteredBy
                )
                result = (
                    result
                    + ' onclick="planetRemove(this,'
                    + str(planet.planetID)
                    + ","
                    + str(self.spawnID)
                    + ",'"
                ...
                result = (
                    result
                    + ' planetUnavailable" onclick="planetAdd(this,'
                    + str(planet.planetID)
                    + ","
                    + str(self.spawnID)
                    + ",'"
    ...
    def getHTML(
            ...
            if self.percentStats.ER != None or formatStyle == 0:
                ...
                if self.stats.ER != None and self.percentStats.ER != None:
                    statVals = (
                        statVals
                        + "<td class='"
                        + ghShared.percOfRangeColor(self.percentStats.ER)
                        + "'><span>"
                        + str(self.stats.ER)
                        + "<br />("
                        + ("%.0f" % float(self.percentStats.ER))
                        + "%)</span></td>"
                    )
            ...
            if self.percentStats.CR != None or formatStyle == 0:
                ...
                if self.stats.CR != None and self.percentStats.CR != None:
                    statVals = (
                        statVals
                        + "<td class='"
                        + ghShared.percOfRangeColor(self.percentStats.CR)
                        + "'><span>"
                        + str(self.stats.CR)
                        + "<br />("
                        + ("%.0f" % float(self.percentStats.CR))
                        + "%)</span></td>"
                    )
            ...
            if self.percentStats.CD != None or formatStyle == 0:
                ...
                if self.stats.CD != None and self.percentStats.CD != None:
                    statVals = (
                        statVals
                        + "<td class='"
                        + ghShared.percOfRangeColor(self.percentStats.CD)
                        + "'><span>"
                        + str(self.stats.CD)
                        + "<br />("
                        + ("%.0f" % float(self.percentStats.CD))
                        + "%)</span></td>"
                    )
            ...
            if self.percentStats.DR != None or formatStyle == 0:
                ...
                if self.stats.DR != None and self.percentStats.DR != None:
                    statVals = (
                        statVals
                        + "<td class='"
                        + ghShared.percOfRangeColor(self.percentStats.DR)
                        + "'><span>"
                        + str(self.stats.DR)
                        + "<br />("
                        + ("%.0f" % float(self.percentStats.DR))
                        + "%)</span></td>"
                    )
            ...
            if self.percentStats.FL != None or formatStyle == 0:
                ...
                if self.stats.FL != None and self.percentStats.FL != None:
                    statVals = (
                        statVals
                        + "<td class='"
                        + ghShared.percOfRangeColor(self.percentStats.FL)
                        + "'><span>"
                        + str(self.stats.FL)
                        + "<br />("
                        + ("%.0f" % float(self.percentStats.FL))
                        + "%)</span></td>"
                    )
            ...
            if self.percentStats.HR != None or formatStyle == 0:
                ...
                if self.stats.HR != None and self.percentStats.HR != None:
                    statVals = (
                        statVals
                        + "<td class='"
                        + ghShared.percOfRangeColor(self.percentStats.HR)
                        + "'><span>"
                        + str(self.stats.HR)
                        + "<br />("
                        + ("%.0f" % float(self.percentStats.HR))
                        + "%)</span></td>"
                    )
            ...
            if self.percentStats.MA != None or formatStyle == 0:
                ...
                if self.stats.MA != None and self.percentStats.MA != None:
                    statVals = (
                        statVals
                        + "<td class='"
                        + ghShared.percOfRangeColor(self.percentStats.MA)
                        + "'><span>"
                        + str(self.stats.MA)
                        + "<br />("
                        + ("%.0f" % float(self.percentStats.MA))
                        + "%)</span></td>"
                    )
            ...
            if self.percentStats.PE != None or formatStyle == 0:
                ...
                if self.stats.PE != None and self.percentStats.PE != None:
                    statVals = (
                        statVals
                        + "<td class='"
                        + ghShared.percOfRangeColor(self.percentStats.PE)
                        + "'><span>"
                        + str(self.stats.PE)
                        + "<br />("
                        + ("%.0f" % float(self.percentStats.PE))
                        + "%)</span></td>"
                    )
            ...
            if self.percentStats.OQ != None or formatStyle == 0:
                ...
                if self.stats.OQ != None and self.percentStats.OQ != None:
                    statVals = (
                        statVals
                        + "<td class='"
                        + ghShared.percOfRangeColor(self.percentStats.OQ)
                        + "'><span>"
                        + str(self.stats.OQ)
                        + "<br />("
                        + ("%.0f" % float(self.percentStats.OQ))
                        + "%)</span></td>"
                    )
            ...
            if self.percentStats.SR != None or formatStyle == 0:
                ...
                if self.stats.SR != None and self.percentStats.SR != None:
                    statVals = (
                        statVals
                        + "<td class='"
                        + ghShared.percOfRangeColor(self.percentStats.SR)
                        + "'><span>"
                        + str(self.stats.SR)
                        + "<br />("
                        + ("%.0f" % float(self.percentStats.SR))
                        + "%)</span></td>"
                    )
            ...
            if self.percentStats.UT != None or formatStyle == 0:
                ...
                if self.stats.UT != None and self.percentStats.UT != None:
                    statVals = (
                        statVals
                        + "<td class='"
                        + ghShared.percOfRangeColor(self.percentStats.UT)
                        + "'><span>"
                        + str(self.stats.UT)
                        + "<br />("
                        + ("%.0f" % float(self.percentStats.UT))
                        + "%)</span></td>"
                    )
        ...
        if formatStyle == 2:
            result += (
                '  <div id="cont_'
                + self.spawnName
                + '" class="boxBorderHidden" style="'
                + resBoxStyle
                + '"'
                + titleStr
                + " onmouseover=\"$(this).removeClass('boxBorderHidden');$(this).addClass('listSelected');\" onmouseout=\"$(this).removeClass('listSelected');$(this).addClass('boxBorderHidden');\">"
            )
        else:
            result += (
                '  <div id="cont_'
                + self.spawnName
                + '" class="resourceBox" style="'
                + resBoxStyle
                + '"'
                + titleStr
                + ">"
        ...
        if statsMatter and self.overallScore != None and self.overallScore > 0:
            result += (
                '  <div class="compareInfo"><span>Quality: '
                + str("%.0f" % (float(self.overallScore)))
                + "</span></div>"
            )
        # resource title row
        if formatStyle == 0:
            result += (
                '  <div style="text-align:left;"><div class="inlineBlock" style="width:55%;text-align:left;"><span style="font-size: 12px;font-weight: bold;"><a href="'
                + ghShared.BASE_SCRIPT_URL
                + "resource.py/"
                + str(self.spawnGalaxy)
                + "/"
                + self.spawnName
                + '" class="nameLink">'
                + self.spawnName
            ...
            result += (
                '    <div style="margin-bottom:4px;text-align:left;"><span style="font-size: 12px;"><a href="'
                + ghShared.BASE_SCRIPT_URL
                + "resource.py/"
                + str(self.spawnGalaxy)
                + "/"
            ...
            result += (
                '    <div style="margin-bottom:4px;text-align:left;"><span style="font-size: 12px;font-weight: bold;"><a href="'
                + ghShared.BASE_SCRIPT_URL
                + "resource.py/"
                + str(self.spawnGalaxy)
                + "/"
        ...
        if currentUser != "":
            if formatStyle != 2 and (
                reputation >= ghShared.MIN_REP_VALS["EDIT_RESOURCE_STATS_TYPE"]
                or currentUser == self.enteredBy
                or admin
            ):
                result += (
                    '  <a alt="Edit Stats" style="cursor: pointer;" onclick="editStats(this, \''
                    + self.spawnName
                    + "');\">[Edit]</a>"
                )
            if formatStyle != 1 and (
                ...
                if formatStyle == 2:
                    result += (
                        '  <div style="width:100px;float:right;"><input type="checkbox" id="chkRemove_'
                        + self.spawnName
                        + '" />Remove</div>'
                    )
                else:
                    result += (
                        '  <a alt="Mark Unavailable" style="cursor: pointer;" onclick="markUnavailable(this, \''
                        + self.spawnName
                        + "', "
                        + str(self.spawnGalaxy)
                        + unPlanetStr
        ...
        if formatStyle == 0:
            result += (
                '  <span style="color:#000033;"><a href="'
                + ghShared.BASE_SCRIPT_URL
                + "resourceType.py/"
                + self.resourceType
                + '" title="View recent spawns of this type" class="nameLink">'
                + self.resourceTypeName
                + "</a></span></div>"
            )
            result += (
                '  <div class="inlineBlock" style="margin-top:2px;margin-right:4px;width:44%;text-align:right;float:right;">'
                + self.getPlanetBar()
                + "</div>"
            )
            result += (
                '  </div><div><div style="height:32px;float:left;"><img src="/images/resources/'
                + self.containerType
                + '.png"/></div>'
            )
        elif formatStyle == 1:
            ...
            result += "    <div>"
            result += (
                '      <div style="height:32px;float:left;"><img src="/images/resources/'
                + self.containerType
                + '.png" /></div>'
            )
            result += (
                '      <div style="width:90px;float:left;"><a href="'
                + ghShared.BASE_SCRIPT_URL
                + "resourceType.py/"
                + self.resourceType
                + '" title="View schematics and recent spawns of this type" class="nameLink">'
                + self.resourceTypeName
                + "</a></div>"
            )
        else:
        ...
        if formatStyle != 2:
            result += (
                '  <table class="resAttr" style="cursor: pointer;" onclick="addResourceCopy(\''
                + self.spawnName
                + '\')" data-resource="'
                + self.spawnName
                + '" data-stats="'
            ...
            if currentUser != "":
                ...
                if self.units > 0:
                    result += (
                        '<div title="Click to toggle favorite group" style="float:right;width:32px;height:32px;margin:4px;cursor:pointer;background-image: url(\'/images/'
                        + favImage
                        + "');background-size: 32px 32px;background-repeat: no-repeat;\" onclick=\"favoritePicker(this, '"
                        + str(self.spawnID)
                        + "', "
                        + str(self.spawnGalaxy)
                        + ", '"
                        ...
                        + ghShared.getNumberAbbr(self.units)
                        + "</span></div>"
                    )
                else:
                    result += (
            ...
            if formatStyle == 0:
                ...
                if self.maxWaypointConc != None:
                    result += (
                        ...
                        + "/"
                        + self.spawnName
                        + '"><img src="/images/waypointMarker.png" alt="waypoint marker" title="waypoint(s) available (best is '
                        + str(self.maxWaypointConc)
                        + '%)" width="20" /></a></div>'
                    )
                # entered
                result += (
                    '  <div class="inlineBlock" style="width:33%;float:right;"><img src="/images/circleBlue16.png" alt="Entered" title="Entered" /><span style="vertical-align:4px;">'
                    + ghShared.timeAgo(self.entered)
                    + ' ago by <a href="'
                    + ghShared.BASE_SCRIPT_URL
                    + "user.py/"
                ...
                if self.verified != None:
                    result += (
                        '  <img src="/images/checkGreen16.png" alt="Verified" title="Verified" /><span style="vertical-align:4px;">'
                        + ghShared.timeAgo(self.verified)
                        + ' ago by <a href="'
                        + ghShared.BASE_SCRIPT_URL
                        + "user.py/"
                        + self.verifiedBy
                    ...
                    if (
                        ...
                        result += (
                            '  <span id="cont_verify_'
                            + self.spawnName
                            + '"><img src="/images/checkGrey16.png" alt="Not Verified" title="Not Verified" /><span style="vertical-align:4px;"><a alt="Verify Resource" style="cursor: pointer;" onclick="quickAdd(null, \''
                            + self.spawnName
                            + "');\">[Verify]</a></span></span>"
                        )
                # unavailable
                result += (
                    '  </div><div class="inlineBlock" style="width:32%;float:right;">'
                )
                if self.unavailable != None:
                    result += (
                        '  <img src="/images/xRed16.png" alt="Unavailable" title="Unavailable" /><span style="vertical-align:4px;">'
                        + ghShared.timeAgo(self.unavailable)
                        + ' ago by <a href="'
                        + ghShared.BASE_SCRIPT_URL
                        + "user.py/"
                        + self.unavailableBy
                ...
                result += (
                    '    <div style="width: 248px;clear:both;margin-left:64px; padding:2px 0px;">'
                    + self.getPlanetBar()
                    + "</div>"
                )
        else:
            if (
                ...
                result += (
                    '  <div id="cont_verify_'
                    + self.spawnName
                    + '" style="width:100px;float:right;"><input type="checkbox" id="chkVerify_'
                    + self.spawnName
                    + '" />Verify</div>'
                )
    ...
    def getMobileHTML(self, currentUser, reputation, admin):
        ...
        if currentUser != "" and (
            ...
            result += (
                '  <span title="Mark Unavailable" style="cursor: pointer;float:right;" onclick="markUnavailable(this, \''
                + self.spawnName
                + "', "
                + str(self.spawnGalaxy)
                + ",'all');\"> [X]</span>"
    ...
    def getRow(self, editable):
        ...
        if editable:
            result += (
                "<td><input type='text' id='units_"
                + str(self.spawnID)
                + "' size='10' maxlength='13' tag='"
                + str(self.units)
                + "' value='"
                + str(self.units)
                + "' onblur='updateFavoriteAmount(this, \""
                + str(self.spawnID)
                + '",this.value);\' onkeyup=\'if($(this).attr("tag")==this.value){$(this).css("color","black");}else{$(this).css("color","red");}\' /></td>'
            )
        else:
            result += (
                "<td><span id='units_"
        ...
        if editable:
            result += (
                "<td style='width:20px'><input type='checkbox' id='chkMove_"
                + str(self.spawnID)
                + "' /></td>"
            )
    ...
    def getStatList(self):
        ...
        if self.stats.ER != None:
            statList += (
                "<span class='altText'>Entangle Resistance:</span> "
                + str(self.stats.ER)
                + "<br />"
            )
        if self.stats.CR != None:
            statList += (
                "<span class='altText'>Cold Resistance:</span> "
                + str(self.stats.CR)
                + "<br />"
            )
        if self.stats.CD != None:
            statList += (
                "<span class='altText'>Conductivity:</span> "
                + str(self.stats.CD)
                + "<br />"
            )
        if self.stats.DR != None:
            statList += (
                "<span class='altText'>Decay Resistance:</span> "
                + str(self.stats.DR)
                + "<br />"
            )
        if self.stats.FL != None:
            statList += (
                "<span class='altText'>Flavor:</span> " + str(self.stats.FL) + "<br />"
            )
        if self.stats.HR != None:
            statList += (
                "<span class='altText'>Heat Resistance:</span> "
                + str(self.stats.HR)
                + "<br />"
            )
        if self.stats.MA != None:
            statList += (
                "<span class='altText'>Malleability:</span> "
                + str(self.stats.MA)
                + "<br />"
            )
        if self.stats.PE != None:
            statList += (
                "<span class='altText'>Potential Energy:</span> "
                + str(self.stats.PE)
                + "<br />"
            )
        if self.stats.OQ != None:
            statList += (
                "<span class='altText'>Overall Quality:</span> "
                + str(self.stats.OQ)
                + "<br />"
            )
        if self.stats.SR != None:
            statList += (
                "<span class='altText'>Shock Resistance:</span> "
                + str(self.stats.SR)
                + "<br />"
            )
        if self.stats.UT != None:
            statList += (
                "<span class='altText'>Unit Toughness:</span> "
                + str(self.stats.UT)
                + "<br />"
            )
        return statList
    ...
    def getInventoryObject(self):
        ...
        result = (
            "<div id='resInventory"
            + str(self.spawnName)
            + "' class='inventoryItem inlineBlock' style='background-image:url(/images/resources/inventory/inv_"
            + self.inventoryType
            + ".png);background-size:64px 64px;' tag='"
            + self.groupList
            + ","
            + self.resourceType
            + ",'>"
        ...
        result += (
            "<div style='float:right;'>" + ghShared.getNumberAbbr(self.units) + "</div>"
        )
        result += (
            "<p style='display:none;'>Loaded with: "
            + self.spawnName
            + ", "
            + self.resourceTypeName
            + "<br />"
        ...
        result += (
            "<div id='stackDetails"
            + str(self.spawnID)
            + "' style='display:none;' class='resourceDetails' tag='ER:"
            + str(self.stats.ER)
            + ",CR:"
            + str(self.stats.CR)
            + ",CD:"
        ...
        result += (
            "  <span class='altText'>Resource Type:</span> " + self.spawnName + "<br />"
        )
        result += (
            "  <span class='altText'>Resource Quantity:</span> "
            + str(self.units)
            + "<br />"
        )
        result += (
            "  <span class='altText'>Resource Class:</span> "
            + self.resourceTypeName
            + "<br />"
        )
        result += self.getStatList()
    ...
    def getAlertsRow(self):
        ...
        # resource title row
        result += (
            '    <td style="width:90px;"><span style="font-size: 12px;font-weight: bold;"><a href="'
            + ghShared.BASE_SCRIPT_URL
            + "resource.py/"
            + str(self.spawnGalaxy)
            + "/"
    ...
    def getSurveyRow(self):
        ...
        result += (
            '  <td><img style="width:16px;height:16px;cursor:pointer;" src="/images/resources/underground_liquid.png" title="Click to toggle favorite group" onclick="favoritePicker(this, \''
            + str(self.spawnID)
            + "', "
            + str(self.spawnGalaxy)
            + ", '"
    ...
    def getHarvestRow(self):
        ...
        result += (
            '  <td><img style="width:16px;height:16px;cursor:pointer;" src="/images/'
            + harvImage
            + '" title="Click to toggle favorite group" onclick="favoritePicker(this, \''
            + str(self.spawnID)
            + "', "
            + str(self.spawnGalaxy)
            + ", '"
        ...
        result += (
            '  <td><span style="font-size: 12px;font-weight: bold;"><a href="'
            + ghShared.BASE_SCRIPT_URL
            + "resource.py/"
            + str(self.spawnGalaxy)
            + "/"
