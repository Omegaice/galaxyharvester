...
if errstr == "":
    ...
    if logged_state > 0:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT alertTypes FROM tFilters WHERE galaxy="
            + str(galaxy)
            + " AND userID='"
            + currentUser
            + "' AND rowOrder="
        ...
        if str(alertType) == "-1":
            ...
            if row != None:
                cursor2 = conn.cursor()
                tempSQL = (
                    "DELETE FROM tFilters WHERE userID='"
                    + currentUser
                    + "' AND galaxy="
                    + str(galaxy)
                    + " AND rowOrder="
            ...
            if row != None:
                ...
                cursor.execute(
                    "SELECT Max(rowOrder) FROM tFilters WHERE galaxy="
                    + str(galaxy)
                    + " AND userID='"
                    + currentUser
                    + "';"
                ...
                cursor2 = conn.cursor()
                tempSQL = (
                    "INSERT INTO tFilters (userID, galaxy, rowOrder, fltType, fltValue, alertTypes, CRmin, CDmin, DRmin, FLmin, HRmin, MAmin, PEmin, OQmin, SRmin, UTmin, ERmin) VALUES ('"
                    + currentUser
                    + "', "
                    + str(galaxy)
                    + ", "
