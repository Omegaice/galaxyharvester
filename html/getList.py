...
if listType == "resource_type_sp":
    listResult = getArrayList(
        listType,
        "SELECT resourceType, resourceTypeName, specificPlanet FROM tResourceType WHERE enterable>0 ORDER BY resourceTypeName;",
    )
...
if listType == "resource_type_group":
    listResult = getArrayList(
        listType, "SELECT resourceType, resourceGroup FROM tResourceTypeGroup;"
    )

if listType == "galaxy":
    listResult = getArrayList(
        listType,
        'SELECT galaxyID, galaxyName, CASE WHEN galaxyState=1 THEN "Active" ELSE "Inactive" END FROM tGalaxy WHERE galaxyState < 3 ORDER BY galaxyState, galaxyName;',
    )
