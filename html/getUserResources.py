...
if resGroup != "any" and resGroup != "":
    joinStr = (
        ...
        + resGroup
        + "' GROUP BY resourceType) rtg ON tResourceType.resourceType = rtg.resourceType"
    )
...
if resGroups != "" and resTypes == "":
    joinStr = (
        joinStr
        + " INNER JOIN (SELECT resourceType FROM tResourceTypeGroup WHERE resourceGroup IN ("
        + resGroups
        + ") GROUP BY resourceType) rtg ON tResourceType.resourceType = rtg.resourceType"
    )
if resGroups != "" and resTypes != "":
...
if uid != "":
    if galaxy.isdigit():
        ...
        if cursor:
            ...
            sqlStr1 += (
                " INNER JOIN tResourceType ON tResources.resourceType = tResourceType.resourceType INNER JOIN tResourceGroup ON tResourceType.resourceGroup = tResourceGroup.resourceGroup INNER JOIN tFavorites ON tResources.spawnID = tFavorites.itemID"
                + joinStr
            )
            sqlStr1 += (
                ' WHERE userID="'
            ...
            while row != None:
                if currentGroup != row[35] and formatType == "":
                    # print group table
                    result += (
                        '<h2 class="categoryHead" style="margin-top:10px;" tag="'
                        + currentGroup
                        + '">'
                    ...
                    result += (
                        '<table id="tbl_'
                        + currentGroup
                        + '" style="width:770px;" class="resourceStats">'
                    )
                    result += getTableHeader(
                        sortBy, uid == currentUser and logged_state == 1
                    )
                    result += groupResult
                    result += "</table>"
                    currentGroup = row[35]
        ...
        if groupCount > 0 and formatType == "":
            ...
            result += (
                '<table id="tbl_'
                + currentGroup
                + '" style="width:760px;" class="resourceStats">'
            )
