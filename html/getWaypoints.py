    ...
    if cursor:
        ...
        if wpID == "":
            if uweeks.isdigit() and uweeks > 0:
                setCriteria += (
                    " AND (tResources.unavailable IS NULL OR DATEDIFF(NOW(),tResources.unavailable) <= "
                    + str(int(uweeks) * 7)
                    + ")"
                )
            elif sortType == "spawn":
        ...
        if len(wpID) > 0:
            ...
            setCriteria += (
                " AND tWaypoint.planetID=(SELECT planetID FROM tPlanet WHERE galaxy IN (0, "
                + galaxy
                + ') AND LOWER(REPLACE(planetName," ","")) = "'
                + planet
                + '")'
            )
        else:
        ...
        wpSql = (
            "SELECT waypointID, spawnName, owner, concentration, lattitude, longitude, waypointType, waypointName, price, shareLevel, planetName, tWaypoint.entered, tResources.resourceType, resourceTypeName, (SELECT Count(userID) FROM tUserEvents ue WHERE ue.targetType='w' AND ue.targetID=waypointID AND ue.eventType='r') AS delCount, (SELECT Count(userID) FROM tUserEvents ue WHERE ue.targetType='w' AND ue.targetID=waypointID AND ue.eventType='v') AS verCount FROM tWaypoint LEFT JOIN tResources ON tWaypoint.spawnID = tResources.spawnID LEFT JOIN tResourceType ON tResources.resourceType = tResourceType.resourceType INNER JOIN tPlanet ON tWaypoint.planetID = tPlanet.planetID WHERE (CASE WHEN tResources.galaxy IS NULL THEN "
            + galaxy
            + " ELSE tResources.galaxy END)="
            + galaxy
            + setCriteria
            + sortString
            + rowLimitStr
