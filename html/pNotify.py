...
if not verify_ipn(data):
    ...
    cursor.execute(
        "SELECT payerEmail FROM tPayments WHERE transactionID=%s;", (transactionID,)
    )
    row = cursor.fetchone()
    ...
    if row != None:
        ...
        if paymentStatus == "Completed":
            ...
            completedStrVal = "NOW(), "
        tempSQL = (
            "INSERT INTO tPayments ("
            + completedStrCol
            + "recvEmail, recvID, payerEmail, payerID, payerStatus, firstName, lastName, paymentStatus, paymentGross, paymentFee, shippingAmount, handlingAmount, currency, paymentDate, transactionType, addressCity, addressCountryCode, addressState, addressStatus, addressStreet, addressZip, customInfo, transactionID) VALUES ("
            + completedStrVal
