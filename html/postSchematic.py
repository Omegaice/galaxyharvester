...
def addSchematicFromLua(conn, skillGroup, luaSchematic, luaObject, galaxy, userID):
    ...
    checkCursor.execute(
        "SELECT schematicName FROM tSchematic WHERE schematicID=%s;", [schemID]
    )
    ...
    if badSchem == False:
        ...
        if len(missingKeys) > 0:
            result = "{0} Some keys were missing so this data may need to be manually edited: {1}".format(
                result, missingKeys
            )
        ...
        try:
            ...
            result = "{0}  Could not determine ingredient data from schematic lua file.  Ingredients will need to entered manually or delete schematic, correct lua file, and try again.".format(
                result
            )
        ...
        if len(ingredients) == len(ingObjects):
            ...
            result = "{0}  Count of ingredientTitleNames and resourceTypes does not match.".format(
                result
            )
        ...
        try:
            expProps = (
                schem["experimentalSubGroupTitles"].strip("{").strip("}").split(",")
            )
            expGroups = (
                schem["experimentalGroupTitles"].strip("{").strip("}").split(",")
            )
            expCounts = (
                schem["numberExperimentalProperties"].strip("{").strip("}").split(",")
            )
            # res data
            expResProps = (
                schem["experimentalProperties"].strip("{").strip("}").split(",")
            )
            expResWeights = (
                schem["experimentalWeights"].strip("{").strip("}").split(",")
            )
            # git rid of whitespace that may be present after items
            ...
            result = "{0}  No schematic qualities data could be extracted from the object lua.  Qualities will need to be entered manually or delete schematic, correct lua file and try again.".format(
                result
            )
        ...
        if expProps != None:
            ...
            for i in range(len(expProps)):
                ...
                if expProps[i] != "null":
                    ...
                    cursor.execute(
                        qualSQL, [schemID, expProps[i], expGroups[i], weightTotal]
                    )
                ...
                for i2 in range(int(expCounts[i])):
                    if expResProps[weightPos + i2] != "XX":
                        ...
                        cursor.execute(
                            weightSQL,
                            [
                                expResProps[weightPos + i2],
                                expResWeights[weightPos + i2],
                            ],
                        )
...
def addSchematicByCopy(
    conn, skillGroup, sourceSchematic, schematicName, galaxy, userID
):
    newSchemID = generateSchematicID(schematicName, galaxy)
    checkCursor = conn.cursor()
    checkCursor.execute(
        "SELECT schematicName FROM tSchematic WHERE schematicID=%s;", [newSchemID]
    )
    checkRow = checkCursor.fetchone()
    checkCursor.close()
    if checkRow == None:
        ...
        result = "Error: The name you are providing is very similar to an existing schematic resulting in a duplicate id.  Please look at at schematic {0} do make sure you are not adding a duplicate or make the name more unique.".format(
            checkRow[0]
        )
...
def addSchematicSkeleton(conn, skillGroup, schematicName, galaxy, userID):
    ...
    checkCursor = conn.cursor()
    checkCursor.execute(
        "SELECT schematicName FROM tSchematic WHERE schematicID=%s;", [schematicID]
    )
    checkRow = checkCursor.fetchone()
    checkCursor.close()
    if checkRow == None:
...
def updateSchematic(conn, schematicID, schematic):
    ...
    cursor.close()
    return "Schematic Updated.  Ingredients {0} (was {1}), Qualities {2} (was {3}), Weights {4} (was {5}).".format(
        ingredientsAfter,
        ingredientsBefore,
        qualitiesAfter,
        qualitiesBefore,
...
if forceOp == "edit":
    ...
    if profession.isdigit() == False:
        errstr = (
            errstr + "Error: Profession ID must be provided to add a new schematic."
        )
    if len(skillGroup) < 3:
    ...
    if userReputation < ghShared.MIN_REP_VALS["ADD_SCHEMATIC"] and not admin:
        errstr = (
            errstr
            + "Error: You have not yet built enough reputation to add schematic data."
        )
    if galaxy.isdigit() == False or len(galaxy) > 5:
        errstr = (
            errstr
            + "Error: A valid numeric galaxy id must be provided to add the new schematic to."
        )
    if re.search("[><&]", schematicName):
        errstr = errstr + "Error: Schematic name contains illegal characters."

if detailsMethod == "DetailsLua":
    if "schematicLua" not in form:
        errstr = "No schematic lua file sent."
    else:
        luaSchematic = form["schematicLua"]
        if not luaSchematic.file:
            errstr = "schematic lua data is not a file."

    if "objectLua" not in form:
        luaObject = ""
    else:
        luaObject = form["objectLua"]
        if not luaObject.file:
            errstr = "object lua data is not a file."
...
if errstr == "":
    ...
    if forceOp != "edit":
        ...
        checkCursor.execute(
            "SELECT profName FROM tProfession WHERE profID=%s", [profession]
        )
        checkRow = checkCursor.fetchone()
        if checkRow == None:
            errstr = (
                errstr + "Error: Cannot find the profession for the id you provided."
            )
        checkRow = None
        # check for valid skill group
        checkCursor.execute(
            "SELECT skillGroupName FROM tSkillGroup WHERE skillGroup=%s", [skillGroup]
        )
        checkRow = checkCursor.fetchone()
        if checkRow == None:
        ...
        if errstr == "":
            if detailsMethod == "DetailsLua":
                results = addSchematicFromLua(
                    conn, skillGroup, luaSchematic, luaObject, galaxy, currentUser
                ).split("|")
                updateSchemID = results[0]
                result = results[1]
            elif detailsMethod == "DetailsCopy":
                checkCursor.execute(
                    "SELECT schematicName FROM tSchematic WHERE schematicID=%s;",
                    [copyFromSchem],
                )
                row = checkCursor.fetchone()
                checkCursor.close()
                if row != None:
                    results = addSchematicByCopy(
                        conn,
                        skillGroup,
                        copyFromSchem,
                        schematicName,
                    ...
                    errstr = (
                        errstr
                        + "Error: Could not find the schematic requested to copy from."
                    )
            else:
                results = addSchematicSkeleton(
                    conn, skillGroup, schematicName, galaxy, currentUser
                ).split("|")
                updateSchemID = results[0]
                result = results[1]
            ...
            if result.find("Error:") > -1:
                ...
                dbShared.logSchematicEvent(
                    ...
                    currentUser,
                    "a",
                    "Added new schematic {0} using {1}.".format(
                        schematicName, detailsMethod
                    ),
                    "history",
                )
    else:
        ...
        checkCursor.execute(
            "SELECT enteredBy, (SELECT customSchematicID FROM tSchematicOverrides WHERE galaxyID=%s AND schematicID=%s) AS customID FROM tSchematic WHERE schematicID=%s",
            [galaxy, schematicID, schematicID],
        )
        checkRow = checkCursor.fetchone()
        if checkRow != None:
            if (
                checkRow[0] != currentUser
                and userReputation < ghShared.MIN_REP_VALS["EDIT_OTHER_SCHEMATIC"]
                and not admin
            ):
                errstr = (
                    errstr
                    + "Error: You do not have high enough reputation to edit other users' schematic data yet."
                )
                ...
                if checkRow[1] == None:
                    checkCursor.execute(
                        "SELECT schematicName, skillGroup FROM tSchematic WHERE schematicID=%s;",
                        [schematicID],
                    )
                    ...
                    if lookRow != None:
                        ...
                        if result.find("Error:") > -1:
                            ...
                            checkCursor.execute(
                                "INSERT INTO tSchematicOverrides (schematicID, galaxyID, customSchematicID) VALUES (%s, %s, %s);",
                                [schematicID, galaxy, updateSchemID],
                            )
                    else:
                        errstr = (
                            errstr
                            + "Error: Could not find the schematic requested to edit from."
                        )
                else:
                    errstr = (
                        errstr
                        + "Error: That schematic is part of the core game schematics and cannot be edited."
                    )
        else:
            errstr = (
                errstr + "Error: Schematic with that ID could not be found for editing."
            )
        ...
        if errstr == "":
            ...
            if result.find("Error:") > -1:
                ...
                dbShared.logSchematicEvent(
                    0, galaxy, updateSchemID, currentUser, "e", result, "history"
                )
...
if errstr != "":
    if forceOp == "edit":
        ...
        print(
            "Location: /message.py?action=addschematicfail&actionreason="
            + urllib.parse.quote(errstr)
        )
        print("")
else:
    if forceOp == "edit":
