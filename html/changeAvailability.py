...
if logged_state > 0:
    ...
    if cursor:
        ...
        if spawnID == "":
            if galaxy != "":
                cursor.execute(
                    "SELECT spawnID, resourceType, CR, CD, DR, FL, HR, MA, PE, OQ, SR, UT, ER, enteredBy FROM tResources WHERE galaxy="
                    + galaxy
                    + ' AND spawnName="'
                    + spawnName
                    + '";'
        ...
        if galaxy == "":
            cursor.execute(
                "SELECT galaxy FROM tResources WHERE spawnID=" + spawnID + ";"
            )
            row = cursor.fetchone()
        ...
        if spawnName == "":
            cursor.execute(
                "SELECT spawnName, enteredBy FROM tResources WHERE spawnID="
                + spawnID
                + ";"
            )
            row = cursor.fetchone()
            ...
            if dbShared.galaxyState(galaxy) == 1:
                ...
                if availability == "0":
                    ...
                    if row == None:
                        ...
                        result = "Marked re-available on " + ghNames.getPlanetName(
                            planets
                        )
                        availability = -1
                ...
                if (
                    int(stats[2]) < repVal
                    and (enteredBy != currentUser or availability == "1")
                    and not admin
                ):
