...
def getUnavailableTypes(resGroup):
    ...
    if cursor:
        sqlStr1 = (
            'SELECT tResourceType.resourceType, resourceTypeName, ur.maxent FROM tResourceType LEFT JOIN (SELECT tResources.resourceType AS resType, Max(entered) AS maxent FROM tResources INNER JOIN tResourceType rt ON tResources.resourceType = rt.resourceType WHERE unavailable IS NULL AND rt.resourceGroup="'
            + resGroup
            + '" GROUP BY tResources.resourceType) ur ON tResourceType.resourceType = ur.resType WHERE enterable = 1 AND resourceGroup="'
            + resGroup
            + '";'
        )
...
def getTypeGroups(resType):
    ...
    if cursor:
        sqlStr1 = (
            'SELECT groupLevel, groupName, tResourceTypeGroup.resourceGroup FROM tResourceTypeGroup INNER JOIN tResourceGroup ON tResourceTypeGroup.resourceGroup = tResourceGroup.resourceGroup WHERE tResourceTypeGroup.resourceType="'
            + resType
            + '";'
        )
        cursor.execute(sqlStr1)
...
if resGroup != "any" and resGroup != "":
    joinStr = (
        joinStr
        + " INNER JOIN (SELECT resourceType FROM tResourceTypeGroup WHERE resourceGroup='"
        + resGroup
        + "' GROUP BY resourceType) rtgg ON rt1.resourceType = rtgg.resourceType"
    )
if resCategory != "any" and resCategory != "":
    joinStr = (
        ...
        + " INNER JOIN (SELECT resourceType FROM tResourceTypeGroup WHERE resourceGroup='"
        + resCategory
        + "' GROUP BY resourceType) rtgc ON rt1.resourceType = rtgc.resourceType"
    )
...
if unavailableDays != None and unavailableDays.isdigit():
    if unavailableDays == "0":
        ...
        if (resType != "any" and resType != "") or (
            resGroup != "any" and resGroup != ""
        ):
            criteriaStr = (
                "tResources.galaxy="
                + galaxy
                + " AND (unavailable IS NULL OR DATEDIFF(NOW(),unavailable) <= "
                + unavailableDays
                + ")"
            )
        else:
...
if planet != "any" and planet != "null" and planet != "" and planet != "0":
    ...
    criteriaStr = (
        criteriaStr
        + " AND EXISTS (SELECT planetID FROM tResourcePlanet WHERE spawnID = tResources.spawnID AND planetID="
        + planet
...
if logged_state == 1:
    ...
    if favorite == "on":
        joinStr = (
            joinStr
            + " INNER JOIN (SELECT itemID, favGroup, units FROM tFavorites WHERE userID='"
            + currentUser
        ...
        joinStr = (
            joinStr
            + ' LEFT JOIN (SELECT itemID, favGroup, units FROM tFavorites WHERE userID="'
            + currentUser
            + '" AND favType=1) favs ON tResources.spawnID = favs.itemID'
        )
...
if errorStr == "":
    ...
    if cursor:
        ...
        sqlStr1 += (
            " rt1.containerType, verified, verifiedBy, unavailable, unavailableBy, rg1.groupName, rt1.resourceCategory, rg2.groupName AS categoryName, rt1.resourceGroup, (SELECT Max(concentration) FROM tWaypoint WHERE tWaypoint.spawnID=tResources.spawnID AND ("
            + wpCriteria
            + ")) AS wpMaxConc"
            + favCols
            + " FROM tResources INNER JOIN tResourceType rt1 ON tResources.resourceType = rt1.resourceType INNER JOIN tResourceGroup rg1 ON rt1.resourceGroup = rg1.resourceGroup INNER JOIN tResourceGroup rg2 ON rt1.resourceCategory = rg2.resourceGroup"
            + joinStr
            + " WHERE "
            + criteriaStr
        )
            ...
            if groupType == 0:
                if currentGroup != row[35]:
                    ...
                    print(
                        ...
                        + row[35]
                        + '</a><span style="font-weight:normal;font-size:.9em;opacity:0.8;"> '
                        + missingStr
                        + "</span></h3>"
                    )
                    print(
            ...
            # group by full tree of groups
                ...
                for i in range(7):
                    ...
                    if currentGroups[i] != typeGroups[i]:
                        print(
                            '<tr><td><div class="surveyGroup" style="margin-left:'
                            + str(i * 16)
                            + 'px;"><a href="'
                            + ghShared.BASE_SCRIPT_URL
                            + "resourceType.py/"
                            + typeGroups[i][: typeGroups[i].find("|")]
                            + '" title="View schematics and recent spawns of this type" class="nameLink" style="color:#999999;">'
                            + typeGroups[i][typeGroups[i].find("|") + 1 :]
                            + "</a></div></td></tr>"
                        )
