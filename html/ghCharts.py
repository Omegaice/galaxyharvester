...
def getChartURL(chartType, chartSize, dataType, resType, available, groupType, galaxy):
    chartStr = (
        "http://chart.apis.google.com/chart?cht=" + chartType + "&chs=" + chartSize
    )
    chartData = "&chd=t:"
    chartLabels = ""
    chartMax = 0
    ...
    if cursor:
        if groupType == "user":
            ...
            if resType == "all":
                ...
                sqlStr = (
                    "SELECT userID, Count(eventTime) AS numActions FROM tResourceEvents mt INNER JOIN tResources ON mt.spawnID = tResources.spawnID INNER JOIN tResourceTypeGroup ON tResources.resourceType = tResourceTypeGroup.resourceType WHERE resourceGroup = '"
                    + resType
                    + "'"
                    + galaxyCriteria
                )
                ...
                sqlStr += (
                    " AND eventType='"
                    + dataType
                    + "' GROUP BY userID ORDER BY Count(eventTime) DESC LIMIT 5;"
                )
        else:
            ...
            sqlStr = (
                "SELECT "
                + dateSelect
                + " AS eDate, Count(eventTime) AS numActions FROM tResourceEvents mt INNER JOIN tResources ON mt.spawnID = tResources.spawnID INNER JOIN tResourceTypeGroup ON tResources.resourceType = tResourceTypeGroup.resourceType WHERE resourceGroup = '"
                + resType
                + "'"
                + galaxyCriteria
                + " AND eventType='"
                + dataType
                + "' AND eventTime > DATE_SUB(CURDATE(),INTERVAL "
                + str(available)
                + " DAY) GROUP BY "
                + dateGroup
                + " ORDER BY "
