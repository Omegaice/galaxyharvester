...
def logEvent(sqlStr, eventType, user, galaxy, spawnID):
    ...
    if cursor:
        ...
        cursor.execute(
            "SELECT userID FROM tUserStats WHERE userID='"
            + user
            + "' AND galaxy="
            + str(galaxy)
            + ";"
        ...
        if row == None:
            cursor.execute(
                "INSERT INTO tUserStats (userID, galaxy, added, planet, edited, removed, verified, waypoint, repGood, repBad) VALUES ('"
                + user
                + "',"
                + str(galaxy)
                + ",0,0,0,0,0,0,0,0);"
        ...
        if eventType == "a":
            cursor.execute(
                "UPDATE tGalaxy SET totalSpawns = totalSpawns + 1 WHERE galaxyID="
                + str(galaxy)
                + ";"
            )
            cursor.execute(
                "UPDATE tUserStats SET added = added + 1 WHERE userID = '"
                + user
                + "' AND galaxy="
                + str(galaxy)
                + ";"
            ...
            cursor.execute(
                "UPDATE tUserStats SET planet = planet + 1 WHERE userID = '"
                + user
                + "' AND galaxy="
                + str(galaxy)
                + ";"
            ...
            cursor.execute(
                "UPDATE tUserStats SET edited = edited + 1 WHERE userID = '"
                + user
                + "' AND galaxy="
                + str(galaxy)
                + ";"
            ...
            cursor.execute(
                "UPDATE tUserStats SET removed = removed + 1 WHERE userID = '"
                + user
                + "' AND galaxy="
                + str(galaxy)
                + ";"
            ...
            markAll = 0
            cursor.execute(
                "SELECT enteredBy FROM tResourcePlanet WHERE spawnID="
                + str(spawnID)
            ...
            cursor.execute(
                "UPDATE tUserStats SET verified = verified + 1 WHERE userID = '"
                + user
                + "' AND galaxy="
                + str(galaxy)
                + ";"
            ...
            cursor.execute(
                "UPDATE tGalaxy SET totalWaypoints = totalWaypoints + 1 WHERE galaxyID="
                + str(galaxy)
                + ";"
            )
            cursor.execute(
                "UPDATE tUserStats SET waypoint = waypoint + 1 WHERE userID = '"
                + user
                + "' AND galaxy="
                + str(galaxy)
                + ";"
            ...
            cursor.execute(
                "UPDATE tUserStats SET repGood = repGood + 1 WHERE userID = '"
                + user
                + "' AND galaxy="
                + str(galaxy)
                + ";"
            ...
            cursor.execute(
                "UPDATE tUserStats SET repBad = repBad + 1 WHERE userID = '"
                + user
                + "' AND galaxy="
                + str(galaxy)
                + ";"
    ...
    if (
        ...
        or eventType == "n"
        or eventType == "g"
        or (eventType == "p" and "previously unavailable" in sqlStr)
    ):
...
def logUserEvent(user, galaxy, targetType, targetID, eventType):
    ...
    if cursor:
        ...
        if (eventType == "n" or eventType == "g") and expGood == True:
            ...
            cursor.execute(
                "SELECT userID, eventTime FROM tResourceEvents WHERE spawnID=%s AND eventType IN ('a','{0}') ORDER BY eventTime DESC;".format(
                    eventType
                ),
                targetID,
            )
...
def logSchematicEvent(
    spawnID, galaxy, schematicID, expGroup, eventType, eventDetail, serverBestMode
):
    ...
    try:
        cursor.execute(
            "INSERT INTO {0} (galaxy, eventTime, eventType, schematicID, expGroup, spawnID, eventDetail) VALUES (%s, NOW(), %s, %s, %s, %s, %s);".format(
                eventTable
            ),
            [galaxy, eventType, schematicID, expGroup, spawnID, eventDetail],
        )
...
def getUserAttr(user, attr):
    ...
    cursor = conn.cursor()
    cursor.execute(
        'SELECT pictureName, emailAddress, themeName, created, inGameInfo, paidThrough, defaultAlertTypes, sharedInventory, sharedRecipes FROM tUsers WHERE userID="'
        + user
        + '"'
    )
    row = cursor.fetchone()
...
def getUserStats(user, galaxy):
    ...
    if galaxy == 0:
        cursor.execute(
            "SELECT Sum(added), Sum(planet), Sum(edited), Sum(removed), Sum(verified), Sum(waypoint), Sum(repGood), Sum(repBad) FROM tUserStats WHERE userID = %s",
            [user],
        )
    else:
        cursor.execute(
            "SELECT added, planet, edited, removed, verified, waypoint, repGood, repBad FROM tUserStats WHERE userID = %s AND galaxy = %s",
            [user, galaxy],
        )
    row = cursor.fetchone()
    if row != None and row[0] != None:
...
def getUserAdmin(conn, user, galaxy):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT roleType FROM tGalaxyUser WHERE userID=%s AND galaxyID=%s ORDER BY roleType;",
        [user, galaxy],
    )
    row = cursor.fetchone()
...
def getGalaxyAdminList(conn, userID):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT tGalaxyUser.galaxyID, galaxyName FROM tGalaxyUser INNER JOIN tGalaxy ON tGalaxyUser.galaxyID = tGalaxy.galaxyID WHERE tGalaxyUser.userID='{0}' AND roleType='a' ORDER BY galaxyName;".format(
            userID
        )
    )
    row = cursor.fetchone()
...
def getPlanetID(planetName):
    ...
    ###
    cursor.execute(
        'SELECT planetID FROM tPlanet WHERE LOWER(REPLACE(planetName," ","")) = "'
        + planetName.lower()
        + '";'
    )
    row = cursor.fetchone()
...
def galaxyState(galaxyID):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT galaxyState FROM tGalaxy WHERE galaxyID = " + str(galaxyID) + ";"
    )
    row = cursor.fetchone()
    if row != None and row[0] != None:
        retVal = row[0]
...
def getGalaxyAdmins(conn, galaxyID):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT userID FROM tGalaxyUser WHERE galaxyID=%s AND roleType='a';", [galaxyID]
    )
    row = cursor.fetchone()
    while row != None:
        admins.append(row[0])
...
def friendStatus(uid, ofUser):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT uf1.added, uf2.added FROM tUserFriends uf1 LEFT JOIN tUserFriends uf2 ON uf1.friendID=uf2.userID AND uf1.userID=uf2.friendID WHERE uf1.userID = '"
        + ofUser
        + "' AND uf1.friendID = '"
        + uid
        + "';"
...
def getSpawnID(resName, galaxy):
    ...
    if len(resName) > 0 and len(galaxy) > 0:
        cursor.execute(
            "SELECT spawnID FROM tResources WHERE galaxy="
            + galaxy
            + " AND spawnName='"
            + resName
            + "';"
...
def getSpawnPlanets(conn, spawnID, availableOnly, galaxy):
    ...
    if cursor:
        ...
        sqlStr = (
            "SELECT DISTINCT tPlanet.planetID, planetName, enteredBy FROM tGalaxyPlanet, tPlanet LEFT JOIN (SELECT planetID, enteredBy FROM tResourcePlanet WHERE spawnID="
            + str(spawnID)
            + criteriaStr
            + ") trp ON tPlanet.planetID=trp.planetID WHERE (tPlanet.planetID < 11) OR (tPlanet.planetID = tGalaxyPlanet.planetID AND tGalaxyPlanet.galaxyID = "
            + str(galaxy)
            + ") ORDER BY planetName;"
        )
        cursor.execute(sqlStr)
...
def getResourceTypeID(conn, resourceTypeName):
    ...
    if resourceTypeName in [
        "Hothian Type 1 Amorphous Gem",
        "Hothian Type 2 Amorphous Gem",
        "Hothian Type 1 Crystalline Gem",
        "Hothian Type 2 Crystalline Gem",
    ]:
...
def getUserPostBlockedSecondsRemaining(userID, targetType):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT eventTime FROM tUserEvents WHERE userID = '"
        + userID
        + "' AND eventType='-' AND targetType='"
        + targetType
        + "' AND causeEventType='r';"
    )
    row = cursor.fetchone()
...
def alertNewAbility(conn, userID, abilityKey, galaxy):
    ...
    alertcursor.execute(
        "INSERT INTO tAlerts (userID, alertType, alertTime, alertMessage, alertLink, alertStatus) VALUES ('{0}', 1, NOW(), '{1}', '{2}', 0);".format(
            userID, message, alertLink
        )
    )
    alertcursor.close()
