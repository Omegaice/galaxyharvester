...
def addGalaxy(
    ...
    for planet in galaxyPlanets:
        if planet.isdigit():
            cursor.execute(
                "INSERT INTO tGalaxyPlanet (galaxyID, planetID) VALUES (LAST_INSERT_ID(), %s)",
                [planet],
            )
    ...
    for user in galaxyAdmins:
        if len(user) > 0:
            cursor.execute(
                "INSERT INTO tGalaxyUser (galaxyID, userID, roleType) VALUES (LAST_INSERT_ID(), %s, %s);",
                [user, "a"],
            )
            result = result + cursor.rowcount
...
def updateGalaxy(
    ...
    cursor.execute(
        tempSQL, (galaxyName, galaxyState, galaxyNGE, galaxyWebsite, galaxyID)
    )
    result = cursor.rowcount
    ...
    for planet in galaxyPlanets:
        if planet.isdigit():
            cursor.execute(
                "INSERT INTO tGalaxyPlanet (galaxyID, planetID) VALUES (%s, %s)",
                [galaxyID, planet],
            )
            result = result + cursor.rowcount
    cursor.execute(
        "DELETE FROM tGalaxyUser WHERE galaxyID=%s AND roleType='a';", [galaxyID]
    )
    for user in galaxyAdmins:
        if len(user) > 0:
            cursor.execute(
                "INSERT INTO tGalaxyUser (galaxyID, userID, roleType) VALUES (%s, %s, %s);",
                [galaxyID, user, "a"],
            )
            result = result + cursor.rowcount
    if result < 1:
...
if not len(galaxyName) > 3:
    errstr = (
        errstr + "Error: You must include the Galaxy name longer than 3 letters. \r\n"
    )
if not len(galaxyWebsite) > 7 or not galaxyWebsite.startswith("http"):
    errstr = (
        errstr
        + "Error: You must include a valid website so public server access can be verified.\r\n"
    )
    ...
    if logged_state > 0:
        ...
        if len(galaxy) > 0 and galaxy != "new":
            ...
            if cursor:
                ...
                cursor.execute(
                    "SELECT galaxyState FROM tGalaxy WHERE galaxyID=%s", [galaxy]
                )
                row = cursor.fetchone()
                cursor.close()
            ...
            if cursor:
                cursor.execute(
                    "SELECT galaxyName FROM tGalaxy WHERE galaxyState=0 AND submittedBy=%s",
                    [currentUser],
                )
                row = cursor.fetchone()
                if row != None:
                    errstr = (
                        errstr
                        + "Error: You already have a galaxy waiting for review ({0}).  Please wait until it is approved before submitting another.".format(
                            row[0]
                        )
                    )
                else:
                    cursor.execute(
                        "SELECT galaxyID FROM tGalaxy WHERE galaxyState < 3 AND galaxyName=%s",
                        [galaxyName],
                    )
                    row = cursor.fetchone()
                    if row != None:
                        errstr = (
                            errstr
                            + "Error: A galaxy already exists with that name, please choose a different name.\n"
                        )
                cursor.close()
