...
if galaxyCursor:
    ...
    while galaxyRow != None:
        if galaxyRow[3] == 1 and (
            galaxyRow[2] == None
            or (
                date(galaxyRow[2].year, galaxyRow[2].month, galaxyRow[2].day)
                < date.today()
            )
        ):
            if exportType == "all":
            ...
            f.write(
                ...
                + datetime.fromtimestamp(time.time()).strftime(rfc822time)
                + '">\n'
            )
            fcsv.write(
                '"name","galaxy_id","galaxy_name","enter_date","type_id","type_name","group_id","CR","CD","DR","FL","HR","MA","PE","OQ","SR","UT","ER","unavailable_date","planets"\n'
            )
            ...
            sqlStr1 += (
                " rt1.containerType, verified, verifiedBy, unavailable, unavailableBy, rt1.resourceCategory, galaxyName FROM tResources INNER JOIN tResourceType rt1 ON tResources.resourceType = rt1.resourceType INNER JOIN tGalaxy ON tResources.galaxy = tGalaxy.galaxyID WHERE "
                + criteriaStr
                + " AND galaxy="
                + str(galaxyRow[0])
            )
            ...
            while row != None:
                ...
                resStr += (
                    '<resource_type id="' + row[5] + '">' + row[6] + "</resource_type>"
                )
                ...
                resStr += "<planets>"
                planetSQL = (
                    "SELECT planetName FROM tResourcePlanet INNER JOIN tPlanet ON tResourcePlanet.planetID = tPlanet.planetID WHERE spawnID="
                    + str(row[0])
                    + " AND "
                    + criteriaStr
                )
            ...
            fcsv.close()
            cursor.execute(
                "UPDATE tGalaxy SET lastExport=NOW() WHERE galaxyID="
                + str(galaxyRow[0])
                + ";"
            )
            cursor.close()
