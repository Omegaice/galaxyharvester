    ...
    for line in f:
        ...
        if eqPos != -1:
            ...
            if eqPos != -1:
                ...
                if eqPos != -1:
                    tmpTangible = (
                        line[line.rfind(",") + 1 :].strip().strip(")").strip('"')
                    )
    ...
    for f in os.listdir(tangiblePath):
        if os.path.isdir(tangiblePath + "/" + f) and f != "." and f != "..":
            ...
            for f2 in os.listdir(tangiblePath + "/" + f):
                if (
                    os.path.isdir(tangiblePath + "/" + f + "/" + f2)
                    and f2 != "."
                    and f2 != ".."
                ):
                    ...
                    for f3 in os.listdir(tangiblePath + "/" + f + "/" + f2):
                        if (
                            os.path.isdir(tangiblePath + "/" + f + "/" + f2 + "/" + f3)
                            and f3 != "."
                            and f3 != ".."
                        ):
                            # object sub - sub type dir
                            # print "      Dipping into: " + f3
                            for f4 in os.listdir(
                                tangiblePath + "/" + f + "/" + f2 + "/" + f3
                            ):
                                if f4 == "objects.lua":
                                    tmpFile = open(
                                        tangiblePath
...
def scrapeSchem(f):
    ...
    if badSchem == False:
        ...
        schemSQL = (
            "INSERT INTO tSchematic (schematicID, schematicName, craftingTab, skillGroup, objectType, complexity, objectSize, xpType, xpAmount, objectPath, objectGroup) VALUES ("
            + schemID
            + ","
            + schem["customObjectName"]
            + ","
            + schem["craftingToolTab"]
            + ',"'
            + skillGroup
            + '",'
            + objectType
        ...
        for i in range(len(ingredients)):
            ...
            ingSQL = (
                "INSERT INTO tSchematicIngredients (schematicID, ingredientName, ingredientType, ingredientObject, ingredientQuantity, ingredientContribution) VALUES ("
                + schemID
                + ","
                + ingredients[i]
                + ","
        ...
        if expProps != None:
            ...
            for i in range(len(expProps)):
                ...
                if expProps[i] != "null":
                    qualSQL = (
                        "INSERT INTO tSchematicQualities (schematicID, expProperty, expGroup, weightTotal) VALUES ("
                        + schemID
                        + ","
                        + expProps[i]
                        + ","
                ...
                for i2 in range(int(expCounts[i])):
                    if expResProps[weightPos + i2] != '"XX"':
                        weightSQL = (
                            "INSERT INTO tSchematicResWeights (expQualityID, statName, statWeight) VALUES (LAST_INSERT_ID(),"
...
# main
conn = pymysql.connect(host="localhost", db="swgresource", user="webusr", passwd="")
                ...
                if (
                    ...
                    for f3 in os.listdir(SCHEM_DIR + "/" + f + "/" + f2):
                        if (
                            ...
                            and f3 != ".."
                            and f3 != "objects.lua"
                            and os.path.isdir(SCHEM_DIR + "/" + f + "/" + f2 + "/" + f3)
                            == False
                        ):
                            tmpFile = open(
                                SCHEM_DIR + "/" + f + "/" + f2 + "/" + f3, "r"
