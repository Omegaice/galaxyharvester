    ...
    if badCreature == False:
        ...
        try:
            if creature["boneType"] != None and len(creature["boneType"]) > 4:
                resSQL = (
                    "INSERT INTO tResourceTypeCreature (resourceType, speciesName, maxAmount) VALUES ('"
                    + creature["boneType"]
                    + "_"
                    + planet
                    + "','"
        ...
        try:
            if (
                creature["milk"] != None
                and creature["milk"].isdigit()
                and int(creature["milk"]) > 0
            ):
                resSQL = (
                    "INSERT INTO tResourceTypeCreature (resourceType, speciesName, maxAmount) VALUES ('milk_wild_"
                    + planet
                    + "','"
                    + speciesName
                    + "',"
                ...
                if os.path.isdir(CREAT_DIR + "/" + f + "/" + f2):
                    ...
                    for f3 in os.listdir(CREAT_DIR + "/" + f + "/" + f2):
                        if (
                            f3 != "."
                            and f3 != ".."
                            and os.path.isdir(CREAT_DIR + "/" + f + "/" + f2 + "/" + f3)
                            == False
                        ):
                            tmpFile = open(
                                CREAT_DIR + "/" + f + "/" + f2 + "/" + f3, "r"
