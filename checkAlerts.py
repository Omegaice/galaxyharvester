...
def checkSpawnAlerts(conn, spawnName, alertValue, galaxy, enteredBy, stats, galaxyName):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT userID, alertTypes, CRmin, CDmin, DRmin, FLmin, HRmin, MAmin, PEmin, OQmin, SRmin, UTmin, ERmin, fltType, fltValue, minQuality FROM tFilters WHERE galaxy="
        + str(galaxy)
        + " AND alertTypes > 0 AND ((fltType = 1 AND fltValue = '"
        + alertValue
        + "') OR (fltType = 2 AND '"
        + alertValue
        + "' IN (SELECT resourceType FROM tResourceTypeGroup WHERE resourceGroup=fltValue)))"
    )
    row = cursor.fetchone()
    ...
    while row != None:
        ...
        if row[15] is not None and row[15] > 0:
            ...
            if qualityTotal < row[15]:
                ...
                alertMessage = (
                    " named {0} added to {1} with quality score {2:.0f} for {3}".format(
                        spawnName, galaxyName, qualityTotal, statStr
                    )
                )
        else:
            ...
            if len(statStr) > 1:
                statStr = statStr[:-2]
            if sendAlert:
                alertMessage = " named {0} added to {1} with stats {2}".format(
                    spawnName, galaxyName, statStr
                )
        ...
        if sendAlert:
            ...
            addAlert(
                row[0],
                row[1],
                typeGroup + alertMessage,
                "http://galaxyharvester.net/resource.py/"
                + str(galaxy)
                + "/"
                + spawnName,
...
def checkDespawnAlerts(conn, spawnID, spawnName, galaxyName, unavailable, galaxy):
    cursor = conn.cursor()
    cursor.execute(
        "SELECT userID, despawnAlert FROM tFavorites WHERE itemID={0} AND despawnAlert > 0;".format(
            spawnID
        )
    )
    row = cursor.fetchone()
    while row != None:
        addAlert(
            ...
            + " despawned at "
            + str(unavailable),
            "http://galaxyharvester.net/resource.py/" + str(galaxy) + "/" + spawnName,
...
def checkServerBest(conn, spawnID, spawnName, galaxy, galaxyName):
    ...
    for x in range(len(result[1])):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT tFavorites.userID, defaultAlertTypes, profName FROM tFavorites INNER JOIN tUsers ON tFavorites.userID = tUsers.userID INNER JOIN tProfession ON tFavorites.itemID = tProfession.profID WHERE tFavorites.galaxy={1} AND favType=3 AND itemID={0} GROUP BY tFavorites.userID, defaultAlertTypes, profName;".format(
                result[0][x], galaxy
            )
        )
        row = cursor.fetchone()
        ...
        while row != None:
            addAlert(
                ...
                "".join((row[2], " Server best alert for ", galaxyName)),
            )
            row = cursor.fetchone()
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT tFavorites.userID, defaultAlertTypes, schematicID, schematicName FROM tFavorites INNER JOIN tUsers ON tFavorites.userID = tUsers.userID INNER JOIN tSchematic ON tFavorites.favGroup = tSchematic.schematicID WHERE tFavorites.galaxy={1} AND favType=4 AND favGroup IN ({0}) GROUP BY tFavorites.userID, defaultAlertTypes, schematicID, schematicName;".format(
                schematicStr, galaxy
            )
        )
        row = cursor.fetchone()
        ...
        while row != None:
            addAlert(
                ...
                "".join((row[3], " Server best alert for ", galaxyName)),
            )
            row = cursor.fetchone()
...
def checkDespawnReputation(conn, spawnID, spawnName, entered, galaxy):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT galaxy, userID, eventTime, eventType, planetID, eventDetail FROM tResourceEvents WHERE spawnID={0} ORDER BY eventTime DESC;".format(
            spawnID
        )
    )
    row = cursor.fetchone()
    ...
    if lastEventTime != None and alreadyRemovedFlag == False:
        ...
        if tmpDays > 3:
            ...
            message = (
                "You gained reputation for your contribution to tracking resource "
                + spawnName
                + "!"
            )
...
def main():
    ...
    checkRemovedStart = datetime.fromtimestamp(time.time()).strftime(
        "%Y-%m-%d %H:%M:%S"
    )
    if lastRemovedCheckTime == "":
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT spawnName, galaxy, enteredBy, resourceType, CR, CD, DR, FL, HR, MA, PE, OQ, SR, UT, ER, galaxyName, unavailable, spawnID, entered FROM tResources INNER JOIN tGalaxy ON tResources.galaxy = tGalaxy.galaxyID WHERE unavailable >= '"
            + lastRemovedCheckTime
            + "';"
        )
        row = cursor.fetchone()
        while row != None:
            ...
            unavailable = row[16]
            checkDespawnAlerts(
                conn, row[17], spawnName, galaxyName, unavailable, row[1]
            )
    ...
    if lastAddedCheckTime == "":
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT spawnName, galaxy, enteredBy, resourceType, CR, CD, DR, FL, HR, MA, PE, OQ, SR, UT, ER, galaxyName, unavailable, spawnID FROM tResources INNER JOIN tGalaxy ON tResources.galaxy = tGalaxy.galaxyID WHERE entered >= '"
            + lastAddedCheckTime
            + "' AND galaxyState=1 and unavailable IS NULL ORDER BY entered;"
        )
        row = cursor.fetchone()
        while row != None:
            alertValue = row[3]
            ...
            galaxyName = row[15]
            checkSpawnAlerts(
                conn, spawnName, alertValue, galaxy, enteredBy, stats, galaxyName
            )
...
def retryPendingMail(conn):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT userID, alertTime, alertMessage, alertLink, alertID FROM tAlerts WHERE alertType=2 AND alertStatus=0 and alertTime > '"
        + minTime.strftime("%Y-%m-%d %H:%M:%S")
        + "';"
    )
    row = cursor.fetchone()
