...
if countsCursor:
    ...
    sys.stdout.write(
        "Updating " + str(countsCursor.rowcount) + " galaxies resource counts.\n"
    )
    updateCursor = conn.cursor()
    row = countsCursor.fetchone()
    while row != None:
        updateSQL = (
            "UPDATE tGalaxy SET totalSpawns="
            + str(row[1])
            + " WHERE galaxyID="
            + str(row[0])
            + ";"
    ...
    countsCursor.execute(countsSQL)
    sys.stdout.write(
        "Updating " + str(countsCursor.rowcount) + " galaxies waypoint counts.\n"
    )
    updateCursor = conn.cursor()
    row = countsCursor.fetchone()
    while row != None:
        updateSQL = (
            "UPDATE tGalaxy SET totalWaypoints="
            + str(row[1])
            + " WHERE galaxyID="
            + str(row[0])
            + ";"
